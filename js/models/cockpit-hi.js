...
if(typeof cockpitShell !== "function"){
  console.warn("[cockpit-hi] js/models/cockpit-shell.js not loaded — load it first.");
  return;
}
...
/* ======================================================== A · THE NOSE */
/* The hull is not authored here any more. js/models/cockpit-shell.js lofts it
   from the same nine-station shape as the rival car (tools/f1_shape.py), so
   the bonnet you look over and the cars you race are one shape — they used to
   be a tapered box and a lofted cigar respectively.

   "coaming" and "flank" are skipped: section D rolls a padded, riveted lip
   and its own tapered tub sides, both better than the shell's plain ones. detail:false likewise — the aeroscreen down in
   section C is the good one.

   Everything below is DRESSING, and every z comes from DECK(y) rather than a
   typed number, so it stays welded to the bonnet if the shape ever moves. */
push(...cockpitShell({ eyeH: EYE_H, detail:false, skip:["coaming","flank"] }));

const DECK = y => cockpitShell.deck(y, EYE_H);

/* air intake at the tip — the rival's own mouth (f1_shape: a disc at car-space
   y 2.315, z 0.310, 0.052 x 0.042), carried across into eye space rather than
   re-guessed — hence 0.310 - EYE_H rather than a typed z, so it cannot drift
   off the nose if the seat height is ever retuned. It is small because the
   real section there is small. */
push(
  { k:"ring", c:"chrome", z:2, axis:"y", at:[0, 2.371, 0.310 - EYE_H],
    r:0.062, ri:0.046, sides:12, open:true },
  stud([0, 2.373, 0.310 - EYE_H], 0.048, 12, [1,0,0], [0,0,1], "dark", 1)
);
...
/* filler cap and its leather strap, offset left of the stripe */
{
  const d = DECK(1.06);
  push(
    { k:"ring", c:"chrome", z:3, axis:"z", at:[-d.hw*0.54, 1.06, d.z + 0.004],
      r:0.050, ri:0.038, sides:12, open:true },
    stud([-d.hw*0.54, 1.06, d.z + 0.002], 0.040, 12, [1,0,0], [0,1,0], "hub", 2),
    { k:"box", c:"dark", z:2, at:[-d.hw*0.54, 1.06, d.z], size:[0.022, 0.150, 0.006] }
  );
}

/* three panel seams across the deck, each with a rivet line */
for(const y of [0.92, 1.52, 2.10]){
  const d = DECK(y), hw = d.hw * 0.94;
  push({ k:"quad", c:"dark", z:2, p:[
    [-hw, y-0.008, d.z], [hw, y-0.008, d.z],
    [ hw, y+0.008, d.z], [-hw, y+0.008, d.z] ]});
  push(...rivets([-hw*0.94, y, d.z+0.002], [hw*0.94, y, d.z+0.002],
                 9, 0.011, [1,0,0], [0,1,0], "hub", 3));
}

/* two long rivet lines down the shoulders, following the taper */
{
  const a = DECK(0.80), b = DECK(2.24);
  both(s => push(...rivets([s*a.hw*0.88, 0.80, a.z+0.004],
                           [s*b.hw*0.88, 2.24, b.z+0.004],
...
push(
  ...
  /* aeroscreen frame, and the perspex itself at 30% — the bake canvas keeps
     ...
     THE APEX HEIGHT IS SOLVED, NOT CHOSEN. Whether the nose tip shows over
     the screen is a race between two angles from the eye:

         nose  |z|/y = 0.427/2.31 = 0.185      (the real deck, sloping away)
         frame |z|/y = 0.140/0.620 = 0.226

     frame > nose, so the nose clears it — which is the whole point of having
     a bonnet in view. It used to clear by luck, because the old bonnet was a
     box with a FLAT top; the real hull drops away toward the tip, so the
     margin had to be bought by dropping the screen 0.04. Anything shallower
     than |z| = 0.115 here and the nose vanishes behind the frame. The floor
     is the instrument panel top (0.402H at y=0.562) — this sits at 0.382H,
     20px clear. Move either and re-solve both. */
  { k:"tube", c:"chrome", z:4, sides:6, r:0.010,
    path:[ [-0.215, 0.600, -0.208], [-0.140, 0.614, -0.156],
           [ 0.000, 0.620, -0.140],
           [ 0.140, 0.614, -0.156], [ 0.215, 0.600, -0.208] ] },
  { k:"quad", c:"glass", z:3, a:0.30, p:[
    [-0.215, 0.601, -0.208], [ 0.215, 0.601, -0.208],
    [ 0.215, 0.616, -0.148], [-0.215, 0.616, -0.148] ]},
...
defineModel("cockpit_60s_hi", {
  ...
  anchors : {
    ...
    /* on the deck, not at a typed height — DECK is the hull's own line */
    roundel: [ 0.000, 1.72,  cockpitShell.deck(1.72, EYE_H).z + 0.004 ]
  },
  parts : PA
});
