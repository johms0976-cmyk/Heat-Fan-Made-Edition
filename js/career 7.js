...
function careerActFinalId(map){
  ...
  if(!pick){
    const races = m.nodes.filter(n => ["enemy","drag","elite"].includes(n.type));
...
function showCareerSelect(){
  ...
  el.innerHTML=`<div class="sheet rules">
    ...
    <div class="racerules" style="margin-top:6px">
      ${save?card("car-cont","🗺️","Continue career",`${esc(save.mapName||"Custom map")} · Act ${actDef((save.act|0)+1).roman} · ⛽ ${save.fuel} · $${save.money} · ${(save.stops!=null?save.stops:save.done.length)} stops made`,"Resume ▸"):""}
...
function showActBreak(){
  ...
  const races=nextMap.nodes.filter(n=>["enemy","drag","elite","boss"].includes(n.type)).length;
  ...
  el.innerHTML=`<div class="sheet" style="max-width:520px;text-align:center">
    ...
    <div class="optgroup" style="text-align:left"><div class="olbl">Next: ${actLabel(next)}</div>
      ...
      ${summaryRow("You carry over", `⛽ ${C.fuel} · $${C.money} · ${C.upgrades.length} Upgrade${C.upgrades.length===1?"":"s"} · ${(C.mods||[]).length} mod${(C.mods||[]).length===1?"":"s"}`)}
    </div>
