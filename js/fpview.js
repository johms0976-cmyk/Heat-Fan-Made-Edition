...
const FOG_START   = 4.0;   // spaces before the haze starts
const FOG_MAX     = 0.90;  // how fully things dissolve at max distance
const FOG_K       = 0.20;  // exponential fog density (higher = thicker haze sooner)

/* ---------- chase camera (third person) ----------------------------------
   js/fpview-chase.js owns the preference and the Settings row. If that file
   is missing we fall back to these defaults with `on` off, so nothing here
   changes and the cockpit behaves exactly as before. */
const CHASE = window.FPCHASE || (window.FPCHASE = {
  on      : false,   // false = cockpit (first person), true = chase (third person)
  dist    : 54,      // how far back the eye sits, in board pixels (ROAD_HW is 19)
  up      : 9,       // how far above the road the eye sits, on top of CAM_H
  horizon : 0,       // nudge the horizon line: negative = look further down
  lag     : 6        // how fast the camera swings onto the car's heading
});

/* fog amount for a point d spaces ahead — exponential falloff blending to the
   low-sky haze colour (TT.sky[2], used as FOGH at every call site). */
function fogAt(d){
  const x = d - FOG_START;
  if(x <= 0) return 0;
  return Math.min(1, 1 - Math.exp(-FOG_K * x)) * FOG_MAX;
}

/* ---- consistent light model (Path A · step 5) -----------------------
   One sun, derived from the same heading map that places the sky sun, drives
   road sheen, kerb highlights and prop shading/contact shadows so every
   directional cue agrees. side = -1 (sun hard left) … +1 (hard right);
   r,g,b = the terrain's warm sun colour. Updated once per frame. */
const LIGHT = { side:0, x:0, y:0, r:240, g:231, b:211 };
function updateLight(TT, P){
  const sunX = W/2 - (FP.cam.head*SKY.headPx)%W;
  LIGHT.x = sunX; LIGHT.y = P.horizon;
  LIGHT.side = Math.max(-1, Math.min(1, (sunX - W/2)/(W*0.5)));
  const s = String(TT && TT.sun || "240,231,211").split(",");
  LIGHT.r = (+s[0])||240; LIGHT.g = (+s[1])||231; LIGHT.b = (+s[2])||211;
}
...
/* BAKED TILE — assets/road/asphalt.png, a 512px seamless aggregate tile
   rendered in Blender (Cycles: ~18k chip solids scattered over a unit square
   with every edge-crossing chip duplicated across, lit by one sun). That buys
   real lit micro-relief — stones with a light side, a dark side and a cast
   shadow — which the flat canvas speckle below cannot fake.
   Regenerate: tools/asphalt-gen.py, run inside Blender.

   The bake is higher-contrast than the procedural tile, so it multiplies at
   its own lower strength. Until it loads — or if it 404s — buildAsphalt()
   still supplies the tile, so the road never waits on the network and never
   breaks. */
ROAD_TEX.src         = "assets/road/asphalt.png";
ROAD_TEX.strengthTex = 0.46;          // multiply opacity when the bake is in use
ROAD_TEX._img        = null;          // the baked bitmap, once loaded
ROAD_TEX._size       = ROAD_TEX.tile; // px size of whatever is actually in _pat
(function loadAsphalt(){
  const im = new Image();
  im.onload = ()=>{
    if(!im.naturalWidth) return;
    ROAD_TEX._img  = im;
    ROAD_TEX._tex  = im;              // createPattern takes an <img> directly
    ROAD_TEX._size = im.naturalWidth;
    ROAD_TEX._pat  = null;            // force a rebuild from the bitmap
  };
  im.onerror = ()=>{ ROAD_TEX._img = null; };   // silently stay procedural
  im.src = ROAD_TEX.src;
})();
...
/* =====================================================================
   GROUND TEXTURE — procedural grass/turf detail on the verges.
   Same idea as the asphalt: a bright, seamless bitmap MULTIPLIED over the
   flat terrain colour, perspective-mapped per row so it's coarse up close
   and fades into the fog. Because it multiplies a near-neutral (faintly
   green) tile, it reads as grass over green terrain but only lightly
   flecks sand/snow tracks instead of dyeing them green. It is painted on
   the two verges OUTSIDE the gravel edge, so it never touches the tarmac.
   Tunables on GRASS_TEX (exposed on window). GR_FAR is how far out to the
   side (board-px) the textured ground reaches — past the screen edge.
   ===================================================================== */
const GRASS_TEX = {
  on      : true,     // master switch
  maxD    : 8.0,      // only texture verge rows nearer than this (fog hides the rest)
  strength: 0.55,     // base multiply opacity at the camera (fades to 0 with fog)
  across  : 5.0,      // texture repeats across the verge span
  along   : 2.2,      // texture repeats per world space along the road
  tile    : 256,      // source texture size (px)
  far     : ROAD_HW + KERB_W + GRAVEL_W + 210,  // lateral reach of the textured ground
  /* per-surface strength scaler — grass carries the most detail; snow and
     concrete stay whisper-subtle so they don't look noisy */
  mix     : { grass:1.0, sand:0.85, snow:0.55, hard:0.5 },
  _tex    : null,     // { surface: offscreen canvas } (built lazily, cached)
  _pat    : null      // { surface: CanvasPattern }   (built lazily, cached)
};
try { window.GRASS_TEX = GRASS_TEX; } catch(e){}

/* pick a surface family from a terrain's near ground colour, so the pack (and
   any future terrain) needs no new fields — grass, sand/dry-dirt, snow or a
   neutral hard/concrete. Bright ground → snow; green-dominant → grass; warm
   tan → sand; otherwise the greyish hard surfaces. */
function groundSurface(TT){
  const hex = (TT && TT.ground && TT.ground[1]) || "#2c3a24";
  const m = /^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})/i.exec(hex);
  if(!m) return "grass";
  const r = parseInt(m[1],16), g = parseInt(m[2],16), b = parseInt(m[3],16);
  if(Math.max(r,g,b) > 200)     return "snow";   // very bright ground (ice/snow)
  if(g > r && g > b)            return "grass";  // green-dominant
  if(r >= g && r - b > 15)      return "sand";   // warm tan / dry dirt
  return "hard";                                 // grey concrete / asphalt lots
}

/* lazily build + cache the pattern for one surface family */
function groundPattern(g, surf){
  if(!GRASS_TEX._pat) GRASS_TEX._pat = {};
  if(!GRASS_TEX._tex) GRASS_TEX._tex = {};
  if(GRASS_TEX._pat[surf]) return GRASS_TEX._pat[surf];
  if(!GRASS_TEX._tex[surf]){
    GRASS_TEX._tex[surf] = surf === "sand" ? buildSandDetail()
                         : surf === "snow" ? buildSnowDetail()
                         : surf === "hard" ? buildAggregateDetail()
                         :                   buildGroundDetail();
  }
  const p = g.createPattern(GRASS_TEX._tex[surf], "repeat");
  GRASS_TEX._pat[surf] = p || null;
  return p;
}

/* seamless turf: bright base (multiply barely darkens), large soft clumps
   for low-frequency patchiness, then fine blade speckle for grain. Every
   mark is wrapped in a 3x3 grid so the tile repeats without a seam. */
function buildGroundDetail(){
  const S = GRASS_TEX.tile;
  const c = document.createElement("canvas"); c.width = c.height = S;
  const g = c.getContext("2d");
  g.fillStyle = "#c9cdb6"; g.fillRect(0,0,S,S);            // bright, faintly-green base
  const wrap = (fn)=>{ for(const dx of [-S,0,S]) for(const dy of [-S,0,S]) fn(dx,dy); };
  // large soft clumps — lighter & darker green plus the odd dry-tan patch
  for(let i=0;i<52;i++){
    const x=Math.random()*S, y=Math.random()*S, r=26+Math.random()*82, roll=Math.random();
    const col = roll<0.40 ? "rgba(150,170,108,0.17)"     // fresh light green
              : roll<0.76 ? "rgba(92,110,66,0.17)"       // shadowed darker green
              :             "rgba(178,168,116,0.13)";    // dry tan patch
  ...
  // fine blade speckle — many tiny near-vertical strokes in varied greens
  const blades=(n,col,len)=>{ g.strokeStyle=col; g.lineWidth=1; g.lineCap="round";
    for(let i=0;i<n;i++){
      const x=Math.random()*S, y=Math.random()*S;
      const a=-Math.PI/2 + (Math.random()-0.5)*0.7, L=len*(0.6+Math.random());
      wrap((dx,dy)=>{ g.beginPath(); g.moveTo(x+dx,y+dy);
        g.lineTo(x+dx+Math.cos(a)*L, y+dy+Math.sin(a)*L); g.stroke(); }); } };
  blades(950, "rgba(66,84,44,0.30)",  4.6);               // dark blades
  blades(720, "rgba(150,172,102,0.28)",4.0);              // light blades
  blades(300, "rgba(188,176,118,0.20)",3.4);              // dry blades
  // scattered darker tufts / clover dots for a bit of body
  g.fillStyle="rgba(56,72,38,0.30)";
  for(let i=0;i<130;i++){ const x=Math.random()*S, y=Math.random()*S, r=1+Math.random()*2.2;
    wrap((dx,dy)=>{ g.beginPath(); g.arc(x+dx,y+dy,r,0,7); g.fill(); }); }
  return c;
}

/* seamless SAND / dry-dirt: warm bright base, soft dune mottle and dense fine
   grain — no blades. Reads as sand or baked earth when multiplied over the
   terrain's tan ground. */
function buildSandDetail(){
  const S = GRASS_TEX.tile;
  const c = document.createElement("canvas"); c.width = c.height = S;
  const g = c.getContext("2d");
  g.fillStyle = "#dcd0b4"; g.fillRect(0,0,S,S);            // warm light base
  const wrap = (fn)=>{ for(const dx of [-S,0,S]) for(const dy of [-S,0,S]) fn(dx,dy); };
  // soft low-frequency drift — lighter cream and darker tan patches
  for(let i=0;i<44;i++){
    const x=Math.random()*S, y=Math.random()*S, r=30+Math.random()*90;
    const col = Math.random()<0.5 ? "rgba(232,222,190,0.14)" : "rgba(150,128,86,0.14)";
    wrap((dx,dy)=>{ const rg=g.createRadialGradient(x+dx,y+dy,0,x+dx,y+dy,r);
      rg.addColorStop(0,col); rg.addColorStop(1,"rgba(0,0,0,0)");
      g.fillStyle=rg; g.beginPath(); g.arc(x+dx,y+dy,r,0,7); g.fill(); });
  }
  // dense fine grain — tiny dark and light granules
  const grain=(n,size,col)=>{ g.fillStyle=col; for(let i=0;i<n;i++){
    const x=Math.random()*S, y=Math.random()*S, r=size*(0.5+Math.random());
    wrap((dx,dy)=>{ g.beginPath(); g.arc(x+dx,y+dy,r,0,7); g.fill(); }); } };
  grain(1300, 0.9, "rgba(120,100,64,0.34)");              // dark specks
  grain(900,  0.9, "rgba(240,232,206,0.30)");             // light specks
  grain(120,  1.8, "rgba(96,80,52,0.30)");                // scattered pebbles
  return c;
}

/* seamless SNOW: near-white cool base, gentle drift mottle and sparse sparkle.
   Kept very low contrast so snow stays smooth — the mix scaler thins it more. */
function buildSnowDetail(){
  const S = GRASS_TEX.tile;
  const c = document.createElement("canvas"); c.width = c.height = S;
  const g = c.getContext("2d");
  g.fillStyle = "#eef2f7"; g.fillRect(0,0,S,S);           // cool near-white base
  const wrap = (fn)=>{ for(const dx of [-S,0,S]) for(const dy of [-S,0,S]) fn(dx,dy); };
  // faint blue-grey drift so the snow isn't perfectly flat
  for(let i=0;i<30;i++){
    const x=Math.random()*S, y=Math.random()*S, r=34+Math.random()*90;
    const col = Math.random()<0.5 ? "rgba(255,255,255,0.14)" : "rgba(176,196,214,0.12)";
    wrap((dx,dy)=>{ const rg=g.createRadialGradient(x+dx,y+dy,0,x+dx,y+dy,r);
      rg.addColorStop(0,col); rg.addColorStop(1,"rgba(0,0,0,0)");
      g.fillStyle=rg; g.beginPath(); g.arc(x+dx,y+dy,r,0,7); g.fill(); });
  }
  // sparse sparkle: bright pin-points, plus a few faint blue shadow dimples
  const dot=(n,size,col)=>{ g.fillStyle=col; for(let i=0;i<n;i++){
    const x=Math.random()*S, y=Math.random()*S, r=size*(0.5+Math.random());
    wrap((dx,dy)=>{ g.beginPath(); g.arc(x+dx,y+dy,r,0,7); g.fill(); }); } };
  dot(260, 0.8, "rgba(255,255,255,0.55)");                // crystal sparkle
  dot(180, 1.1, "rgba(168,190,210,0.20)");                // soft shadow dimples
  return c;
}

/* seamless AGGREGATE: neutral grey concrete/asphalt lot — soft patch mottle
   and a fine spread of light & dark stones, with a couple of very faint
   hairline joints. Subtle by design (paved surfaces read almost flat). */
function buildAggregateDetail(){
  const S = GRASS_TEX.tile;
  const c = document.createElement("canvas"); c.width = c.height = S;
  const g = c.getContext("2d");
  g.fillStyle = "#cfd0d2"; g.fillRect(0,0,S,S);           // neutral light grey base
  const wrap = (fn)=>{ for(const dx of [-S,0,S]) for(const dy of [-S,0,S]) fn(dx,dy); };
  // soft cement patches — slightly lighter and darker greys
  for(let i=0;i<40;i++){
    const x=Math.random()*S, y=Math.random()*S, r=28+Math.random()*80;
    const col = Math.random()<0.5 ? "rgba(232,233,236,0.12)" : "rgba(120,122,128,0.12)";
    wrap((dx,dy)=>{ const rg=g.createRadialGradient(x+dx,y+dy,0,x+dx,y+dy,r);
      rg.addColorStop(0,col); rg.addColorStop(1,"rgba(0,0,0,0)");
      g.fillStyle=rg; g.beginPath(); g.arc(x+dx,y+dy,r,0,7); g.fill(); });
  }
  // exposed-aggregate stones: fine dark and light grey grit
  const grit=(n,size,col)=>{ g.fillStyle=col; for(let i=0;i<n;i++){
    const x=Math.random()*S, y=Math.random()*S, r=size*(0.5+Math.random());
    wrap((dx,dy)=>{ g.beginPath(); g.arc(x+dx,y+dy,r,0,7); g.fill(); }); } };
  grit(1000, 1.0, "rgba(96,98,104,0.34)");                // dark stones
  grit(700,  1.0, "rgba(238,239,242,0.28)");              // light stones
  grit(90,   2.0, "rgba(74,76,82,0.30)");                 // coarse aggregate
  // a couple of very faint expansion joints running across the slab
  g.strokeStyle = "rgba(90,92,98,0.18)"; g.lineWidth = 1;
  for(const jy of [S*0.36, S*0.78]){
    g.beginPath(); g.moveTo(-2, jy); g.lineTo(S+2, jy + (Math.random()-0.5)*6); g.stroke();
  }
  return c;
}

/* multiply the ground detail over the two verge trapezoids of one row (a=near,
   b=far), using the tile chosen for this terrain's surface. Left verge runs
   outer(Lgr) → inner(Lg); right verge inner(Rg) → outer(Rgr).
   Reuses texTri/solveTexAffine (defined below for the road). */
function texGroundRow(g, a, b, fogT, surf){
  if(!GRASS_TEX.on) return;
  const pat = groundPattern(g, surf);
  if(!pat){ GRASS_TEX.on = false; return; }
  const alpha = GRASS_TEX.strength * (GRASS_TEX.mix[surf] ?? 1) * (1 - fogT);
  if(alpha <= 0.02) return;
  const T = GRASS_TEX.tile;
  const uR = GRASS_TEX.across * T;             // inner edge U (outer edge = 0)
  const va = a.sp * GRASS_TEX.along * T;        // near depth V (scrolls with the world)
  const vb = b.sp * GRASS_TEX.along * T;        // far  depth V
  g.save();
  g.globalCompositeOperation = "multiply";
  g.globalAlpha = alpha;
  g.fillStyle = pat;
  let ok = true;
  if(a.Lgr && b.Lgr && a.Lg && b.Lg){          // left verge
    ok = texTri(g, pat, a.Lgr, a.Lg, b.Lg, 0, uR, uR, va, va, vb);
    if(ok) ok = texTri(g, pat, a.Lgr, b.Lg, b.Lgr, 0, uR, 0, va, vb, vb);
  }
  if(ok && a.Rgr && b.Rgr && a.Rg && b.Rg){    // right verge
    ok = texTri(g, pat, a.Rg, a.Rgr, b.Rgr, 0, uR, uR, va, va, vb);
    if(ok) ok = texTri(g, pat, a.Rg, b.Rgr, b.Rg, 0, uR, 0, va, vb, vb);
  }
  g.restore();
  if(!ok) GRASS_TEX.on = false;                // no Pattern.setTransform → fall back to flat
}
...
function texRoadRow(g, a, b, fogT){
  ...
  const alpha = (ROAD_TEX._img ? ROAD_TEX.strengthTex : ROAD_TEX.strength) * (1 - fogT);
  if(alpha <= 0.02) return;
  const pat = ROAD_TEX._pat, T = ROAD_TEX._size;
...
style.textContent = `
  ...
  #fpchip{position:absolute; top:8px; right:10px; z-index:41; opacity:0; pointer-events:none;
          ...
          border-radius:8px; padding:4px 8px; max-width:min(46vw,320px);}
  /* the hint no longer sits on the glass the whole round: it fades up when the
     cockpit is raised and fades back out on its own after a few seconds so it
     never blocks the view. It re-triggers each time the cockpit turns on
     (show() re-adds .on), so the controls stay discoverable. */
  #fpcam.on ~ #fpchip{ animation:fpchipHint 5.5s ease forwards; }
  @keyframes fpchipHint{
    0%{opacity:0} 7%{opacity:.9} 62%{opacity:.9} 100%{opacity:0}
  }
  ...
  #trackwrap.fp-pip.fp-artbg{ background-image:none !important; }
  /* schematic (Mario-Kart-style) mini-map: hide the board graphic in the slot
     but keep the element (opacity:0 still takes taps + animates the zoom) so
     the canvas map shows and tapping still reaches the board/artwork. */
  #trackwrap.fp-live.fp-pip.fp-mini #tracksvg{ opacity:0 !important; }
...
function pipOut(){
  clearTimeout(PIP._t);
  PIP.on = false;
  wrap.classList.remove("fp-mini");                  // restore the board graphic
...
/* spaces from the car to the finish line (the chequered flag total). Used for
   the CORNER tile once there's no corner left before the finish. */
function distToFinish(p){
  try{
    if(typeof raceDist === "function")
      return Math.max(0, Math.ceil(raceDist() - p.total));
  }catch(_){}
  return null;
}
...
function fpGauges(){
  ...
  if(nc === Infinity){
    // No corner left before the finish — the finish line is the last thing to
    // cross. Show the spaces-to-finish in the CORNER tile and put the flag in
    // the LIMIT tile (instead of a flag in CORNER and a dash in LIMIT).
    const df = distToFinish(p);
    if(sv) sv.textContent = (df == null) ? "🏁" : String(df);
    if(hv){ hv.textContent = "🏁"; hv.style.color = ""; }
    return;
  }
  if(sv) sv.textContent = String(Math.max(0, Math.ceil(nc - p.total)));
  if(hv){
...
function makeProjector(cam){
  ...
  const rx = -fy, ry = fx;
  /* FOV widens a touch with speed — the classic dolly-out that makes the
     road rush at you. cam.spd is already smoothed, so this never jitters. */
  const FOCAL = H * 1.05 * (1 - Math.min(0.14, (cam.spd||0)*0.014));
  const horizon = H * 0.40 + cam.bob + (cam.pitch||0) + (cam.hzOff||0);
  /* camera rides the road's elevation; cam.eyeUp lifts it for the chase cam */
  const eyeH = CAM_H + (cam.eyeUp||0) + (cam.elev||0);
  return {
    horizon,
    focal: FOCAL,                      // hook: fpview-poly projects meshes with it
...
function drawProp(g, kind, x, y, u, r){
  /* step 5: sun-directional contact shadow grounding the prop (all kinds) */
  if(u > 0.05){
    const fw = 4*u*(0.8 + r*0.6);
    const sdx = -LIGHT.side * fw*0.7;                       // falls away from the sun
    g.save();
    g.globalAlpha = 0.30 * Math.min(1, u/2.5);
    g.fillStyle = "#0a080c";
    g.beginPath();
    g.ellipse(x + sdx, y, fw*(1 + Math.abs(LIGHT.side)*0.6), Math.max(1, 1.2*u), 0, 0, 7);
    g.fill();
    g.restore();
  }
  switch(kind){
  case "tree":{
    ...
    /* step 5: warm sun-lit crown + shaded far side */
    g.fillStyle = `rgba(${LIGHT.r},${LIGHT.g},${LIGHT.b},.22)`;
    g.beginPath(); g.arc(x+LIGHT.side*cw*0.36, y-th-cw*0.66, cw*0.42, 0, 7); g.fill();
    g.fillStyle = "rgba(0,0,0,.16)";
    g.beginPath(); g.arc(x-LIGHT.side*cw*0.36, y-th-cw*0.42, cw*0.46, 0, 7); g.fill();
    break; }
  case "pine":{
    ...
    /* step 5: warm rim down the sun-facing edge of the spire */
    g.fillStyle = `rgba(${LIGHT.r},${LIGHT.g},${LIGHT.b},.18)`;
    g.beginPath();
    g.moveTo(x+LIGHT.side*cw*0.5, y-th*0.34);
    g.lineTo(x, y-th*0.98); g.lineTo(x+LIGHT.side*cw*0.16, y-th*0.34);
    g.closePath(); g.fill();
    break; }
  case "cactus":{
...
/* soft radial contact shadow — grounds a billboard car on the tarmac so it
   reads as sitting ON the road, not floating a hair above it. A flat ellipse
   looked like a dark pill; this fades out at the rim like a real shadow. */
function softShadow(g, x, y, w){
  const rx = w*0.66, ry = w*0.155;
  const sg = g.createRadialGradient(x, y, 0, x, y, rx);
  sg.addColorStop(0,   "rgba(0,0,0,.44)");
  sg.addColorStop(0.55,"rgba(0,0,0,.26)");
  sg.addColorStop(1,   "rgba(0,0,0,0)");
  g.save();
  g.translate(x, y); g.scale(1, ry/rx); g.translate(-x, -y);
  g.fillStyle = sg; g.beginPath(); g.arc(x, y, rx, 0, 7); g.fill();
  g.restore();
}

function drawRivalSprite(g, sp, x, y, w, glow, brake){
  const s = (RIVAL_SPR.foot * w) / sp.meta.bw;   // width = foot×w on screen
  softShadow(g, x, y, w);                          // grounded contact shadow
...
function drawRival(g, x, y, w, color, glow, rel, brake, o){
  /* hook: flat-shaded polygon car (Settings > Graphics). Fails open. */
  ...
  softShadow(g, fx*0.25, 0, w);                 // grounded contact shadow
...
function drawHood(g, p, steer){
  /* hook: flat-shaded polygon cockpit (Settings > Graphics). Fails open. */
...
function drawWheel(g, steer, p){
  /* hook: flat-shaded polygon wheel (Settings > Graphics). Fails open. */
...
/* =====================================================================
   POST  (Path A · step 4) — a full-frame post pass that ties the
   photographic sky, textured road and rival renders into one image:
     · BLOOM    — bright areas (sky, sun, kerbs, glints) bleed a soft glow.
                  Cheap: downscale the frame, square it to keep only brights,
                  add it back upscaled (bilinear = the blur) with 'lighter'.
     · VIGNETTE — corners darkened via a cached radial (baked once per size).
     · GRAIN/DITHER — a baked noise tile blended 'overlay' at low alpha with a
                  random offset each frame: adds film grain AND breaks the
                  8-bit gradient banding in the sky.
   Runs in device pixels on the finished world, BEFORE the HUD/mini-map so
   those stay crisp. Bloom (the pricey pass) is skipped when frames are slow;
   DPR/QUAL already shrink the buffer under load. Tunables on window.POST.
   ===================================================================== */
const POST = {
  on        : true,
  bloom     : 0.22,   // additive bloom strength (0 = off)
  bloomDown : 5,      // bloom blur downscale (bigger = softer + cheaper)
  vignette  : 0.38,   // corner darkening (0..1)
  grain     : 0.045,  // film-grain / dither strength
  minFrameMs: 30,     // skip bloom when the rolling frame time exceeds this
  _buf:null, _vig:null, _vigKey:"", _noise:null, _npat:null
};
try{ window.POST = POST; }catch(e){}

function postProcess(){
  if(!POST.on) return;
  const dw = cv.width, dh = cv.height;
  ctx.save();
  ctx.setTransform(1,0,0,1,0,0);            // device pixels — ignore camera shake & DPR
  ctx.imageSmoothingEnabled = true;

  /* ---- BLOOM ---- */
  if(POST.bloom > 0 && _frameMs < POST.minFrameMs){
    const bw = Math.max(1, Math.round(dw/POST.bloomDown));
    const bh = Math.max(1, Math.round(dh/POST.bloomDown));
    let buf = POST._buf || (POST._buf = document.createElement("canvas"));
    ...
    bc.drawImage(cv, 0,0,dw,dh, 0,0,bw,bh);         // downscale (area-average blur)
    bc.globalCompositeOperation = "multiply";
    bc.drawImage(buf, 0,0);                          // square → keep only bright areas
    bc.globalCompositeOperation = "source-over";
    ctx.globalCompositeOperation = "lighter";
    ctx.globalAlpha = POST.bloom;
    ctx.drawImage(buf, 0,0,bw,bh, 0,0,dw,dh);       // upscale (bilinear blur) + add
  }

  /* ---- VIGNETTE ---- */
  if(POST.vignette > 0){
    const key = dw+"x"+dh+":"+POST.vignette;
    if(POST._vigKey !== key){
      const vg = POST._vig || (POST._vig = document.createElement("canvas"));
      vg.width = dw; vg.height = dh;
      const q = vg.getContext("2d");
      q.clearRect(0,0,dw,dh);
      const g = q.createRadialGradient(dw/2, dh*0.52, Math.min(dw,dh)*0.30,
                                       dw/2, dh*0.52, Math.max(dw,dh)*0.72);
      g.addColorStop(0,   "rgba(0,0,0,0)");
      g.addColorStop(0.65,"rgba(0,0,0,0)");
      g.addColorStop(1,   `rgba(0,0,0,${POST.vignette})`);
      q.fillStyle = g; q.fillRect(0,0,dw,dh);
      POST._vigKey = key;
    }
    ctx.globalCompositeOperation = "source-over"; ctx.globalAlpha = 1;
    ctx.drawImage(POST._vig, 0,0);
  }

  /* ---- GRAIN + DITHER ---- */
  if(POST.grain > 0){
    if(!POST._npat){
      const n = POST._noise = document.createElement("canvas"); n.width = n.height = 128;
      const q = n.getContext("2d");
      const id = q.createImageData(128,128), d = id.data;
      for(let i=0;i<d.length;i+=4){ const v = 128 + (Math.random()*2-1)*40;
        d[i]=d[i+1]=d[i+2]=v; d[i+3]=255; }
      q.putImageData(id,0,0);
      POST._npat = ctx.createPattern(n, "repeat");
    }
    ctx.globalCompositeOperation = "overlay";   // grey 128 = identity; deviations perturb
    ctx.globalAlpha = POST.grain;
    const ox = (Math.random()*128)|0, oy = (Math.random()*128)|0;
    ctx.translate(-ox, -oy);
    ctx.fillStyle = POST._npat;
...
function render(dt){
  ...
  /* Chase cam: the eye sits BACK down the ribbon from the car, so cam.total
     stays the camera's own position and every distance measured off it —
     road rows, corner boards, fog, crest clipping — keeps working untouched.
     The car itself is then just another car in the scene, drawn at v.total. */
  const chaseOn = !!CHASE.on;
  const sp1  = (typeof SPACING === "number" && SPACING > 0) ? SPACING : 12;
  const back = chaseOn ? Math.max(0.25, CHASE.dist / sp1) : 0;   // in spaces

  cam.total = v.total - back;
  cam.eyeUp = chaseOn ? (CHASE.up || 0) : 0;
  cam.hzOff = chaseOn ? (CHASE.horizon || 0) : 0;
  cam.off += (v.off - cam.off)*Math.min(1, dt*8);
  const here = lerpPose(cam.total); if(!here) return;
  /* aim past the CAR, not past the camera, or the chase view lags into corners */
  const look = lerpPose(cam.total + back + 1.4) || here;
  const targHead = Math.atan2(look.ty + here.ty, look.tx + here.tx);
  if(cam.head == null) cam.head = targHead;
  else cam.head += shortTurn(cam.head, targHead)*Math.min(1, dt*(chaseOn ? (CHASE.lag||6) : 6));
  ...
  const elevAhead = elevAt(cam.total + back + 2.2);
  ...
  const TT = fpTerrain();
  drawSky(ctx, TT, P);
  updateLight(TT, P);     // step 5: one sun drives sheen, kerbs, prop shading
  ...
  for(let d=0.30; d<=VIEW_SPACES; d+=STEP){
    ...
    /* wide ground anchors, well past the screen edge, so the turf texture on
       the verges has a real world-space outer edge to map against */
    const Lgr = P.proj(q.x - q.nx*GRASS_TEX.far, q.y - q.ny*GRASS_TEX.far, hz);
    const Rgr = P.proj(q.x + q.nx*GRASS_TEX.far, q.y + q.ny*GRASS_TEX.far, hz);
    const row = { d, sp: cam.total + d, hz, L, R, Lk, Rk, Lg, Rg, Lgr, Rgr };
  ...
  /* ---------------------------------------------------------------------
     PASS 1 — terrain depth strips only.
     These are full-width fillRects (they lay a fogged land colour at every
     depth so hills read). Because they span the WHOLE width, a NEARER row's
     strip used to be painted AFTER — and therefore ON TOP OF — the tarmac of
     a FARTHER row. On a straight the road stays centred so its own quad
     re-covered it, but through a corner the far road curves off to the side,
     into the band a nearer strip fills, and the grass painted over it. Doing
     every strip first, before any tarmac, removes that overlap entirely.
     --------------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------------
     GRASS PASS — multiply ground detail onto the verges, over the finished
     terrain strips but before any tarmac. The tile is chosen from the
     terrain's surface (grass / sand / snow / concrete), so desert, snow and
     city tracks get the right grain instead of green blades. Kept to the near
     rows and to spare frames, gated on the same budget the road texture uses.
     --------------------------------------------------------------------- */
  const texGrass = GRASS_TEX.on && _frameMs < 30;
  if(texGrass){
    const surf = groundSurface(TT);
    for(let i=rows.length-2; i>=0; i--){
      const a = rows[i], b = rows[i+1];
      if(!a.L || !a.R || !b.L || !b.R) continue;
      if(!a.vis && !b.vis) continue;
      if(a.d >= GRASS_TEX.maxD) continue;
      texGroundRow(ctx, a, b, fogAt(a.d), surf);
    }
  }

  /* ---------------------------------------------------------------------
     PASS 2 — gravel, kerbs, tarmac and every road marking, far→near.
     All of it now lands on top of the completed terrain, so the ribbon
     stays whole through corners.
     --------------------------------------------------------------------- */
  for(let i=rows.length-2; i>=0; i--){
    const a = rows[i], b = rows[i+1];
    if(!a.L || !a.R || !b.L || !b.R) continue;
    ...
    /* step 5: the kerb facing the sun catches a warm highlight */
    if(Math.abs(LIGHT.side) > 0.08){
      const hi = (0.16*Math.abs(LIGHT.side)*(1-fogT)).toFixed(3);
      if(LIGHT.side < 0 && a.Lk && b.Lk){ ctx.fillStyle = `rgba(${LIGHT.r},${LIGHT.g},${LIGHT.b},${hi})`;
        bleedQuad(ctx, a.Lk, a.L, b.L, b.Lk, _bl); }
      else if(LIGHT.side > 0 && a.Rk && b.Rk){ ctx.fillStyle = `rgba(${LIGHT.r},${LIGHT.g},${LIGHT.b},${hi})`;
        bleedQuad(ctx, a.R, a.Rk, b.Rk, b.R, _bl); }
    }
    ...
    /* step 5: warm specular sheen, biased toward the sun instead of centred */
    {
      const c = LIGHT.side * 0.20;                            // slide the streak sunward
      const sh = (0.05*(1 - Math.abs(LIGHT.side)*0.35)*(1-fogT)).toFixed(3);
      ctx.fillStyle = `rgba(${LIGHT.r},${LIGHT.g},${LIGHT.b},${sh})`;
      quad(ctx, spanPt(a,c-0.15), spanPt(a,c+0.15), spanPt(b,c+0.15), spanPt(b,c-0.15));
    }
  ...
  const others = (G.players||[])
    .filter(o => (chaseOn || o !== p) && !(o.finished && o._v && o._v.blend >= 1))
    .map(o => {
  ...
  for(const {o, pr, d} of others){
    ...
    withClip(Math.max(0.31, d), ()=>{
      ctx.save();
      if(o !== p) ctx.globalAlpha *= Math.max(0.05, 1 - fogAt(Math.max(0, d))*0.8);
      drawRival(ctx, pr.x, pr.y, w, o.color || "#888", glow, rel, brake, o);
      ctx.restore();
    });
  }
  ...
  /* ----- speed streaks: rush lines sweeping past the edges + a peripheral
     darkening that tightens with pace, so speed is felt, not just measured -- */
  const streak = Math.min(1, cam.spd/9);
  if(streak > .10){
    ctx.save(); ctx.lineCap = "round";
    ctx.strokeStyle = `rgba(240,231,211,${streak*.28})`;
    for(let k=0;k<12;k++){
      const t = (k*0.11 + cam.total*0.9) % 1;                 // 0 far → 1 near
      const y = P.horizon + (H-P.horizon)*(0.06 + 0.92*t);
      const l = 24 + 160*streak*t;                            // longer up close
      ctx.lineWidth = (0.8 + 2.4*streak) * (0.4 + t);
      ctx.beginPath(); ctx.moveTo(W*0.05, y); ctx.lineTo(W*0.05 + l, y + 3*t); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(W*0.95, y); ctx.lineTo(W*0.95 - l, y + 3*t); ctx.stroke();
    }
    ctx.restore();
    if(streak > .35){                                          // speed-tunnel vignette
      const k = (streak - 0.35)/0.65;
      const vg = ctx.createRadialGradient(W/2, H*0.52, Math.min(W,H)*0.30,
                                          W/2, H*0.52, Math.max(W,H)*0.72);
      vg.addColorStop(0, "rgba(0,0,0,0)");
      vg.addColorStop(1, `rgba(0,0,0,${(0.20*k).toFixed(3)})`);
      ctx.fillStyle = vg; ctx.fillRect(0,0,W,H);
    }
  }
  ...
  if(shim > 0.02){
    /* Cockpit: the band sits between the horizon and the hood. Chase: the car
       occupies that same band, and warping strips of it smears the sprite —
       so the haze is lifted to the road AHEAD of the car instead. */
    const bandTop = Math.round(H*(chaseOn ? 0.41 : 0.50));
    const bandBot = Math.round(H*(chaseOn ? 0.52 : 0.80));
    const strip = 3;
    ctx.save();
  ...
  if(!chaseOn){                       // chase cam looks AT the car, not out of it
  ...
  postProcess();          // Path A step 4: bloom + vignette + grain/dither (world only)
  ...
  if(PIP.on && svgEl){
    const useMini = MINI.on;
    if(wrap.classList.contains("fp-mini") !== useMini) wrap.classList.toggle("fp-mini", useMini);
    if(useMini){
      drawMinimap(p, LAND, pipRect());          // Mario-Kart-style route + dots
    }else{
      drawPipBacking();
...
function drawWeatherFX(WX, P, dt, now){
  ...
  if(d.lowSun){
    /* the sun sits on the horizon and sweeps with your heading (same map the
       sky uses), so turning to face it floods the windshield with glare. */
    let sunX = W/2 - (FP.cam.head*SKY.headPx) % W;
    sunX = ((sunX % W) + W) % W;
    const sy = P.horizon - 6;
    const facing = Math.max(0, 1 - Math.abs(sunX - W/2)/(W*0.5));   // 1 = staring in
    /* warm grade over everything — golden hour even above a blue sky */
    ctx.fillStyle = `rgba(255,196,120,${(0.09 + 0.06*facing).toFixed(3)})`;
    ctx.fillRect(0,0,W,H);
    /* the sun's broad glow, brighter the more square-on you are */
    const glow = ctx.createRadialGradient(sunX, sy, 6, sunX, sy, H*0.85);
    glow.addColorStop(0,   `rgba(255,243,208,${(0.50+0.38*facing).toFixed(3)})`);
    glow.addColorStop(0.15,`rgba(255,224,150,${(0.28+0.30*facing).toFixed(3)})`);
    glow.addColorStop(1,   "rgba(255,224,150,0)");
    ctx.fillStyle = glow; ctx.fillRect(0,0,W,H);
    /* windshield wash + lens-flare beads down the sun→centre axis when you're
       driving into it */
    if(facing > 0.35){
      const wash = (facing-0.35)/0.65;
      ctx.fillStyle = `rgba(255,244,214,${(0.22*wash).toFixed(3)})`;
      ctx.fillRect(0,0,W,H);
      ctx.save(); ctx.globalCompositeOperation = "lighter";
      for(const [t,r,a] of [[0.32,28,0.20],[0.55,15,0.13],[0.80,44,0.10]]){
        const fx = W/2 + (sunX - W/2)*(1 - 2*t), fy = H/2 + (sy - H/2)*(1 - 2*t);
        const fg = ctx.createRadialGradient(fx, fy, 0, fx, fy, r);
        fg.addColorStop(0, `rgba(255,236,192,${(a*wash).toFixed(3)})`);
        fg.addColorStop(1, "rgba(255,236,192,0)");
...
function drawHUD(p, v){
  ...
  /* The gauge ribbon below the windshield already carries GEAR, ENGINE,
     CORNER and LIMIT (renderGauges is retitled/fed by fpGauges). So when the
     ribbon is present we DON'T repeat them on the canvas — the bottom-left
     box shrinks to just SPD (the one figure the ribbon doesn't show), and the
     top plaque is repurposed as a heat-cost warning. On a page with no ribbon
     we fall back to the full cluster so nothing is lost. */
  const hasRibbon = !!(document.getElementById("g-speed") || document.getElementById("g-hand"));

  if(!DOCKED){
    if(hasRibbon){
      const bw = 74, bh = 58, bx = 10, by = H - bh - 10;
      ...
      ctx.fillStyle = THEME.dim;  ctx.font = `700 8px ${mono}`;
      ctx.fillText("SPD", bx+12, by+16);
      ctx.fillStyle = THEME.glow; ctx.font = `800 30px ${mono}`;
      ctx.fillText(String(p.speed||0), bx+12, by+46);
      ctx.restore();
    }else{
  ...
  /* Top-centre plaque — the corner distance/limit lives in the ribbon, so this
     now fires ONLY when your current speed would bust the next corner, and it
     shows the thing you're actually deciding on: the Heat you'd have to pay
     (speed − limit). If the Engine can't cover it, it flips to a SPIN warning. */
  let nxt = null;
  try{ for(const c of cornerTotals()){ if(c > v.total){ nxt = c; break; } } }catch(_){}
  if(nxt != null && (nxt - v.total) <= 24){
    const lim = effLimitAt(p, nxt);
    const over = (lim == null) ? 0 : Math.max(0, (p.speed||0) - lim);
    if(over > 0){
      const distSp = Math.max(0, Math.ceil(nxt - v.total));
      const willSpin = over > (p.engine|0);
      const txt = willSpin
        ? `⚠ SPIN RISK · ${over} OVER · ${p.engine|0} HEAT LEFT`
        : `CORNER in ${distSp} · PAY ${over} HEAT`;
      ctx.save(); ctx.globalAlpha = .96;
      ctx.font = `800 11px ${mono}`;
      const tw = ctx.measureText(txt).width + 26;
      const px = W/2 - tw/2, py = 8;
      ctx.fillStyle = "rgba(13,10,19,.72)"; rr(ctx, px, py, tw, 22, 8);
      ctx.strokeStyle = THEME.heat; ctx.lineWidth = 1.5;
      ctx.strokeRect(px, py, tw, 22);
      const pulse = willSpin ? (0.7 + 0.3*Math.sin(performance.now()/140)) : 1;
...
/* ---- Mario-Kart-style schematic mini-map -----------------------------
   The route ribbon is static per track/size, so it's baked once into an
   offscreen canvas (dark outline + light road + centre dashes + a start
   tick) and blitted each frame; only the coloured car dots and landing
   rings are drawn live on top. Colours come from G.players; your own car
   is drawn last, larger, with a white ring. Toggle on window.MINI.on. */
const MINI = { on:true, _cv:null, _key:"" };
try{ window.MINI = MINI; }catch(e){}

function miniRibbon(mw, mh, sc){
  const Pw = TRACK.spacePts;
  const w = Math.max(1, Math.round(mw)), h = Math.max(1, Math.round(mh));
  const key = (TRACK.id||TRACK.name||"t")+":"+Pw.length+":"+w+":"+h;
  if(MINI._key === key && MINI._cv) return MINI._cv;
  const cv = MINI._cv || (MINI._cv = document.createElement("canvas"));
  cv.width = w; cv.height = h;
  const q = cv.getContext("2d");
  q.clearRect(0,0,w,h);
  const trace = ()=>{ q.beginPath();
    Pw.forEach((p,i)=> i ? q.lineTo(p[0]*sc, p[1]*sc) : q.moveTo(p[0]*sc, p[1]*sc));
    if(typeof LAYOUT==="undefined" || LAYOUT!=="open") q.closePath(); };
  const rib = Math.max(3, mw*0.05);
  q.lineJoin = "round"; q.lineCap = "round";
  trace(); q.strokeStyle = "rgba(0,0,0,.45)"; q.lineWidth = rib+4; q.stroke();   // soft shadow
  trace(); q.strokeStyle = "#232334";        q.lineWidth = rib+2; q.stroke();   // dark kerb
  trace(); q.strokeStyle = "#c7ccd9";        q.lineWidth = rib;   q.stroke();   // road
  trace(); q.strokeStyle = "rgba(92,98,120,.9)"; q.lineWidth = Math.max(1, rib*0.16);
  q.setLineDash([rib*0.5, rib*0.9]); q.stroke(); q.setLineDash([]);             // centre dashes
  if(Pw.length > 1){                                                            // start/finish tick
    const a=Pw[0], b=Pw[1], dx=b[0]-a[0], dy=b[1]-a[1], L=Math.hypot(dx,dy)||1;
    const nx=-dy/L, ny=dx/L, ex=rib*0.95;
    q.strokeStyle = "#fff"; q.lineWidth = Math.max(1.5, rib*0.32);
    q.beginPath(); q.moveTo(a[0]*sc+nx*ex, a[1]*sc+ny*ex);
                   q.lineTo(a[0]*sc-nx*ex, a[1]*sc-ny*ex); q.stroke();
  }
  MINI._key = key;
  return cv;
}
function miniDot(x, y, r, col, hi){
  ctx.beginPath(); ctx.arc(x, y, r + (hi?1.8:1.1), 0, 7);
  ctx.fillStyle = "rgba(0,0,0,.65)"; ctx.fill();              // dark outline for contrast
  ctx.beginPath(); ctx.arc(x, y, r, 0, 7); ctx.fillStyle = col || "#999"; ctx.fill();
  if(hi){ ctx.beginPath(); ctx.arc(x, y, r+2.6, 0, 7);
          ctx.strokeStyle = "#fff"; ctx.lineWidth = 1.8; ctx.stroke(); }
}
function drawMinimap(me, LAND, rect, lineOnly){
  const Pw = TRACK && TRACK.spacePts; if(!Pw || !Pw.length || !TRACK.imgW) return;
  const mw = rect ? rect.w : Math.min(150, W*0.30);
  const mh = mw * (TRACK.imgH/TRACK.imgW);
  const mx = rect ? rect.x : 10, my = rect ? rect.y : 10, sc = mw / TRACK.imgW;
  ctx.save();
  if(!lineOnly){                                              // panel backing
    ctx.globalAlpha = .95;
    ctx.fillStyle = "rgba(13,10,19,.72)";
    rr(ctx, mx-6, my-6, mw+12, mh+12, 10);
    ctx.strokeStyle = THEME.deep; ctx.lineWidth = 1.5;
    ctx.strokeRect(mx-6, my-6, mw+12, mh+12);
  }
  ctx.globalAlpha = lineOnly ? .5 : 1;                        // baked route ribbon
  ctx.drawImage(miniRibbon(mw, mh, sc), mx, my, Math.round(mw), Math.round(mh));
  ctx.globalAlpha = 1;
  if(lineOnly){ ctx.restore(); return; }
  if(LAND && LAND.size){                                      // landing preview rings
  ...
  const dotR = Math.max(2.6, mw*0.03);                        // coloured car dots
  for(const o of (G.players||[])){
    if(o===me) continue;                                     // draw self last, on top
    if(!o._v && o.total==null) continue;
    const pos = carPose(o._v ? o._v.total : o.total, 0); if(!pos) continue;
    miniDot(mx+pos.x*sc, my+pos.y*sc, dotR, o.color, false);
  }
  if(me){
    const pos = carPose(me._v ? me._v.total : me.total, 0);
    if(pos) miniDot(mx+pos.x*sc, my+pos.y*sc, dotR*1.5, me.color, true);
  }
  ctx.restore();
}
...
/* hook: frame state for js/fpview-poly.js (W/H change on resize, so getters) */
try{ window.FPCTX = { get W(){ return W; }, get H(){ return H; },
                      get DPR(){ return DPR; },
                      LIGHT, THEME, P:null, cam:null }; }catch(e){}
