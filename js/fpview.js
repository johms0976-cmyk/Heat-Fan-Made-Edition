...
style.textContent = `
  ...
  #trackwrap.fp-live.fp-pip #tracksvg{
    ...
    border-radius:10px;
    filter:drop-shadow(0 0 26px rgba(0,0,0,.85)) contrast(1.12) saturate(1.06);
  }
...
chip.id = "fpchip";
chip.textContent = "● cockpit — tap road to skip · tap map for artwork · ▦ / V board view · H dash";
...
function show(){
  ...
  chip.textContent = replayOn()
    ...
    : "● cockpit — tap road to skip · tap map for artwork · ▦ / V board view · H dash";
  cv.classList.add("on");
  pipIn();
...
function render(dt){
  ...
  if(PIP.on && svgEl){
    drawPipBacking();
    drawMinimap(p, LAND, PIP.r || pipRect(), true);
  }
