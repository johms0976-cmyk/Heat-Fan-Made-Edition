...
/* raw mesh — a baked vertex/face list, for shapes no primitive can express.

   The escape hatch for geometry authored in a modeller: export the verts
   flat and the faces as index arrays and the whole part arrives in one
   piece. This is what lets js/models/car-f1.js carry a lofted hull.

       { k:"mesh", c:"body", z:1, d:1,
         v:[x,y,z, x,y,z, ...],        // flat, model space, metres
         f:[[0,1,2,3],[4,5,6], ...] }  // indices INTO THIS PART's v

   Indices are part-local, so parts can be reordered or dropped without
   renumbering. Faces may be any size, and an n-gon costs exactly one fill
   — which is why a baked cylinder cap is cheaper here than the `cyl`
   builder's fan.

   Winding is irrelevant: draw() orients every normal toward the viewer.
   One `c` slot covers the whole part, so bake one part per colour. */
function buildMesh(V, F, p){
  const v = p.v, faces = p.f;
  if(!v || !faces) return;
  const base = V.length / 3, n = v.length / 3;
  for(let k=0;k<n;k++) pushV(V, v[k*3], v[k*3+1], v[k*3+2]);
  const c = slot(p.c,"all"), z = p.z || 0;
  for(let k=0;k<faces.length;k++){
    const src = faces[k], m = src.length, idx = new Array(m);
    for(let j=0;j<m;j++) idx[j] = base + src[j];
    F.push({ i:idx, c, z });
  }
}
...
let ORD = [];
/* hoisted so the sort doesn't allocate a fresh comparator each draw */
...
function draw(g, model, o){
  ...
  /* --- depth-sort faces (mean camera-space depth, plus the z bias) ---
     ...
     The slots are allocated ONCE per face-count and then mutated in place.
     Building a fresh {f,d} per face per car per frame is what used to make
     cost scale worse than linearly with face count: at 700 faces x 5 rivals
     that was 3,500 short-lived objects a frame going straight to the GC,
     and it dominated everything else in the draw. Reusing them is free. */
  const nf = F.length;
  if(ORD.length !== nf){
    ORD = new Array(nf);
    for(let f=0; f<nf; f++) ORD[f] = { f:0, d:0 };
  }
  for(let f=0; f<nf; f++){
    const idx = F[f].i, m = idx.length;
    let d = 0;
    for(let j=0;j<m;j++) d += AY[idx[j]];
    const o = ORD[f];
    o.f = f;
    o.d = d/m - (F[f].z||0)*1e3;
  }
