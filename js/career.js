...
const CCOST = { start:{f:0,m:0}, enemy:{f:0,m:0}, drag:{f:0,m:0}, elite:{f:0,m:20}, boss:{f:0,m:40},
  ...
  city:{f:0,m:0}, town:{f:0,m:0}, poi:{f:0,m:0} };
const CNODE_R = { start:16, boss:26, elite:20, city:18, enemy:14, drag:14, grudge:17, trial:15, night:16,
  event:14, merchant:15, chop:15, treasure:15, rest:15, town:14, poi:12 };
...
function sealActMap(map, isLast, idx){
  const existing = map.nodes.find(n => n.data && n.data.actFinal) || map.nodes.find(n => n.type === "boss");
  if(existing){
    existing.type = isLast ? "boss" : (existing.type === "boss" ? "elite" : existing.type);
    existing.data = Object.assign({}, existing.data, { actFinal:true });
    map._finalId = existing.id;
    return map;
  }
  ...
  const gap = Math.max(90, span / Math.max(1, top));
  /* dir "up" means higher floors sit further up the screen (smaller y) */
  const y = (map.dir === "down") ? Math.max(...ys) + gap : Math.min(...ys) - gap;
  const id = "actfinal" + (idx == null ? "" : idx);
  map.nodes.push({ id, type: isLast ? "boss" : "elite", label:"",
                   floor: top + 1, x: Math.round((Math.min(...xs) + Math.max(...xs)) / 2),
                   y: Math.round(y), data:{ actFinal:true } });
  for(const n of topRow) map.edges.push([n.id, id]);
  map.maxFloor = top + 1;
  map._finalId = id;
  return map;
}
...
/* ---- what meeting the goal is worth ------------------------------------
  ...
  const bonus = kind==="boss"     ? {m:140,rep:2,upg:true,legend:true}
              : kind==="actboss"  ? {m:90, rep:1,upg:true,legend:true}
...
/* ---------------- random roadmap — three act maps, one per screen.
   Each act is its own short walk (a handful of floors) rather than a third
   of one long one, so the acts stay legible when the camera fits them. */
function careerGenBook(){
  const maps={};
  for(let a=0;a<CAREER_ACT_COUNT;a++){
    const m=careerGenMap(6+Math.floor(Math.random()*3));
...
function careerGenMap(floorsWanted){
  ...
  for(let attempt=0;attempt<40;attempt++){
    ...
    for(const n of nodes){
      ...
      let t=null,tries=0;
      while(tries++<30){
        const late=k=>["elite","rest","grudge","night"].includes(k);
        let tot=0; for(const k of pool) tot+=(late(k)&&n.floor<elmin?0:W[k]);
        let r=Math.random()*tot;
        for(const k of pool){ const wv=(late(k)&&n.floor<elmin?0:W[k]); if((r-=wv)<0){ t=k; break; } }
        if(!t) t="enemy";
        if(["rest","merchant","chop","elite","grudge","trial","night"].includes(t)&&parents(n.id).some(p=>p&&p.type===t)){ t=null; continue; }
        break;
      }
      n.type=t||"enemy";
    }
  ...
  if(ok){
    $("#crGo").onclick=()=>{
      careerPrizeChain(node,cfg,ok,wonOutright,()=>{
        ...
        if(isActFinal){ showActBreak(); return; }
        showCareerMap();
      });
    };
