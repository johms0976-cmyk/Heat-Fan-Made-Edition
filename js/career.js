...
function newCareer(rawMap){
  ...
  G.career = { v:2, book, mapName:book.name, act:0,
    ...
    chops:0, stressCuts:0, tuning:null, table:{ act:1, rows:[] }, starterCuts:[], carryHeat:null,
...
function showCareerGarage(){
  ...
  box.innerHTML=`<div class="cdetail"><h3>🛠️ ${esc(classByKey(C.cls).name)} — ${defs.length} Upgrade${defs.length===1?"":"s"} fitted</h3>
    <div class="crow" style="color:#bdb3d4">Deck: <b>${careerDeckCount()}</b> cards before Stress${(C.chops|0)?` · ${C.chops} cut out at chop shops`:""}${(C.stressCuts|0)?` · surgery: −${C.stressCuts} Stress`:""}</div>
...
function careerChopMarketHTML(mult){
  ...
  return `${hasUpg
    ...
    <div class="phase-hint" style="min-height:0;margin-top:6px">Next cut: <b style="color:${C.money>=price?"#ffd86b":"#e08a8a"}">$${price}</b>${(C.chops|0)?` · ${C.chops} already cut out — the price rises each time`:""}. Deck: <b>${careerDeckCount()}</b> cards before Stress.</div>`;
}
