...
/* ============================================================
   ...
   Node types → career meaning:
     ...
     event = roadside event · merchant = speed shop · rest = sponsor day
     treasure = prize crate · city/town/poi = "any" (you choose)
   Travelling is free; big races charge a MONEY entry fee.
...
/* ============================================================
   ENGINE HEAT CARRIES BETWEEN RACES
   The Engine no longer refills for free at the start line. Whatever Heat you
   finish a race with is what you keep — plus a single Heat that seeps back in
   overnight (mods, and a chop-shop cool-down, improve on that). The Heat you
   burned stays burned: it rides in your draw pile as dead weight until you
   cool it down again, exactly like it does mid-race.
     C.carryHeat = Engine count at the end of the last race.
                   null / undefined  → start the next race brim-full
                   (a fresh career, or after a chop-shop cool-down).
   Mods: postHeat +n  recovers that many extra Heat between races;
         startFull     ignores carry-over entirely (Engine always starts full).
   ============================================================ */
const CAREER_HEAT_RECOVERY = 1;   // Heat that seeps back between races, before mods
function careerHeatRecovery(){
  const fx = (typeof careerModFx === "function") ? careerModFx() : {};
  return CAREER_HEAT_RECOVERY + (fx.postHeat || 0);
}
function careerStartsFull(){
  const fx = (typeof careerModFx === "function") ? careerModFx() : {};
  return !!fx.startFull;
}

/* ============================================================
   CUTTING THE STOCK SPEED CARDS
   The chop shop used to only touch drafted Upgrade cards. Now the 12 stock
   Speed cards every career starts with (three each of 1–4) can be cut too, so
   a deck can be thinned right down to the fast cards. Cuts are stored as a
   flat list of the Speed values removed (e.g. [1,1,2]) and re-applied to the
   freshly built deck at the start of every race. A floor keeps the car
   drivable — you can't strip the stock deck to nothing.
   ============================================================ */
const STARTER_SPEED = [1,1,1,2,2,2,3,3,3,4,4,4];   // the stock 12 (3× each of 1–4)
const STARTER_MIN   = 6;                            // never thin the stock deck below this
function careerStarterRemaining(){
  const C = G.career, cut = (C && C.starterCuts) || [];
  const rem = STARTER_SPEED.slice();
  for(const v of cut){ const i = rem.indexOf(v); if(i >= 0) rem.splice(i, 1); }
  return rem;   // array of Speed values still in the stock deck
}
/* total non-Stress cards in the career deck: stock Speed left + drafted Upgrades */
function careerDeckCount(){
  return careerStarterRemaining().length + ((G.career && G.career.upgrades) || []).length;
}

/* ============================================================
   CURSE CARDS
   Dead-weight cards a career can pick up from roadside events. Each one is,
   at heart, a 0-Speed card you can NEVER discard — it just clogs the hand the
   way Heat and Stress do, one fewer real card every draw. Nastier ones also
   bite when you're forced to play them: they bleed Heat from the Engine on
   reveal (def.heat) and/or drop your corner speed limits for the turn
   (def.limit, negative). The only cure is paying to have one cut out (chop
   shop) or a roadside event that strips or exorcises it.

   They ride the game's existing Upgrade-card pipeline: a curse in the deck is
   an "upg"-type card whose def carries {curse:true, v:0, heat?, limit?, scrap?}.
   The one thing the engine must know is that a curse can't be discarded — that
   lives in game.js as isCurse()/discardable(). Everything else (movement,
   Heat-on-reveal, corner-limit shift, rendering) is stock Upgrade handling. */
const CAREER_CURSES = [
  {key:"deadweight", name:"Dead Weight",       icon:"🪨",
   def:{curse:true, name:"Dead Weight",       v:0}},
  {key:"cracked",    name:"Cracked Block",     icon:"🩹",
   def:{curse:true, name:"Cracked Block",     v:0, heat:1}},
  {key:"bent",       name:"Bent Chassis",      icon:"📐",
   def:{curse:true, name:"Bent Chassis",      v:0, limit:-1}},
  {key:"brakes",     name:"Worn Brakes",       icon:"🛞",
   def:{curse:true, name:"Worn Brakes",       v:0, limit:-1, heat:1}},
  {key:"clutch",     name:"Slipping Clutch",   icon:"⚙️",
   def:{curse:true, name:"Slipping Clutch",   v:0, heat:2}},
  {key:"steering",   name:"Twitchy Steering",  icon:"🎯",
   def:{curse:true, name:"Twitchy Steering",  v:0, limit:-2}},
  {key:"throttle",   name:"Sticky Throttle",   icon:"🔥",
   def:{curse:true, name:"Sticky Throttle",   v:0, heat:1, scrap:1}},
  {key:"rotor",      name:"Warped Rotor",      icon:"🌀",
   def:{curse:true, name:"Warped Rotor",      v:0, limit:-1, scrap:1}},
  {key:"bearing",    name:"Seized Bearing",    icon:"☠️",
   def:{curse:true, name:"Seized Bearing",    v:0, heat:2, limit:-1}},
  {key:"driveshaft", name:"Snapped Driveshaft",icon:"💀",
   def:{curse:true, name:"Snapped Driveshaft",v:0, heat:1, limit:-2}}
];
function careerCurseByKey(k){ return CAREER_CURSES.find(c => c.key === k) || null; }
/* one-line description of what a curse does when it's out, for shop/event UI */
function careerCurseDesc(cur){
  if(!cur) return "";
  const d = cur.def, bits = [];
  if(d.heat)  bits.push(`bleeds ${d.heat} Heat on reveal`);
  if(d.limit) bits.push(`corner limits ${d.limit} the turn it's played`);
  if(d.scrap) bits.push(`mills ${d.scrap} card${d.scrap>1?"s":""} off your deck`);
  return bits.length ? bits.join(", ") : "a dead 0-Speed card you can't discard";
}
/* pick a curse the player doesn't already have too many of (light de-dupe) */
function careerRollCurse(rng){
  const C = G.career, have = (C && C.curses) || [];
  const r = rng || Math.random;
  const fresh = CAREER_CURSES.filter(c => !have.includes(c.key));
  const pool = fresh.length ? fresh : CAREER_CURSES;
  return pool[Math.floor(r() * pool.length)];
}
/* saddle the career with a curse; returns the curse added (for the toast) */
function careerAddCurse(key){
  const C = G.career; if(!C) return null;
  const cur = key ? careerCurseByKey(key) : careerRollCurse();
  if(!cur) return null;
  (C.curses = C.curses || []).push(cur.key);
  saveCareer();
  return cur;
}
/* keep the deck's Stress reduction in sync with both stress sinks: the pricey
   two-shot surgery (stressCuts) and the ordinary per-card strips bought at the
   cut market or won from events (stressStrip). careerApplyFx reads tuning.stress. */
function careerSyncStressTuning(){
  const C = G.career; if(!C) return;
  const total = (C.stressCuts | 0) + (C.stressStrip | 0);
  C.tuning = Object.assign({}, C.tuning || {}, { stress: -total });
}
/* base Stress cards the career deck carries before any strips — the three
   every deck starts with, plus the Balanced class's extra. This caps how many
   Stress cards there are left to pay to remove. */
function careerBaseStress(){
  const C = G.career; let base = 3;
  if(C && typeof classByKey === "function"){
    const cls = classByKey(C.cls);
    if(cls && cls.stress) base += cls.stress;
  }
  return base;
}
function careerStressRemovable(){ return Math.max(0, careerBaseStress() - (G.career ? (G.career.stressStrip|0) : 0)); }

/* ---- Robustly reach into a driver's deck without knowing game.js's exact
   field names (these two files never import game.js). Fail safe everywhere:
   if the pile can't be found we simply leave the car stock. */
function careerDrawPile(p){
  for(const n of ["deck","draw","drawPile","drawpile","pile","library","stock","cards"])
    if(Array.isArray(p[n])) return p[n];
  let best = null;
  for(const k in p){
    if(["hand","discard","play","playArea","crossings","_passRivals"].includes(k)) continue;
    const a = p[k];
    if(Array.isArray(a) && a.length && a[0] && typeof a[0] === "object" &&
       ("type" in a[0] || "kind" in a[0]) && (!best || a.length > best.length)) best = a;
  }
  return best;
}
function cardValueOf(c){
  if(!c) return null;
  for(const f of ["val","value","speed","n","num","spd"]) if(typeof c[f] === "number") return c[f];
  return null;
}
function isSpeedCardObj(c){
  if(!c) return false;
  if(typeof isHeat   === "function" && isHeat(c))   return false;
  if(typeof isStress === "function" && isStress(c)) return false;
  const t = (c.type || c.kind || "").toString().toLowerCase();
  if(/heat|stress|upg|boost|brake|adren/.test(t)) return false;
  if(c.upg || c.upgrade || c.adv || c.def)        return false;   // looks like an Upgrade
  const v = cardValueOf(c);
  return typeof v === "number" && v >= 1 && v <= 4;
}

/* Called once per human at the start of a career race — after the deck is
   built and careerApplyFx has sized the Engine, before the opening hand is
   drawn. Applies (a) stock Speed-card cuts and (b) the between-race Heat
   carry-over. Never corrupts a deck it can't read: on any doubt it just
   leaves the Engine full. */
function careerPrepHumanCar(p){
  const C = G.career; if(!C) return;
  const draw = careerDrawPile(p);

  /* (a) remove any stock Speed cards the player has paid to cut */
  const cuts = (C.starterCuts || []).slice();
  if(draw && cuts.length){
    for(const v of cuts){
      const i = draw.findIndex(c => isSpeedCardObj(c) && cardValueOf(c) === v);
      if(i >= 0) draw.splice(i, 1);
    }
  }

  /* (a2) shuffle in any curse cards the career is carrying. A curse is a stock
     Upgrade card object whose def is flagged curse:true — game.js scores it,
     charges its Heat, shifts the corner limit and (crucially) refuses to let it
     be discarded. We mint fresh copies every race, so cutting one out at a shop
     is the only way it ever leaves for good. */
  if(draw && C.curses && C.curses.length && typeof makeUpgCard === "function"){
    let added = 0;
    for(const key of C.curses){
      const cur = careerCurseByKey(key);
      if(!cur) continue;
      try { draw.push(makeUpgCard(cur.def)); added++; } catch(e){}
    }
    if(added && typeof shuffle === "function"){
      const sh = shuffle(draw.slice()); draw.length = 0; for(const c of sh) draw.push(c);
    }
    if(added && !p.isBot)
      log(`\uD83D\uDC80 ${added} curse card${added===1?"":"s"} ride${added===1?"s":""} in your deck \u2014 dead weight you can't discard, only pay to have cut out.`, "warn");
  }

  /* (b) carry Heat over instead of refilling the Engine for free */
  const cap = (typeof heatCapFor === "function") ? heatCapFor(p) : (p.engine | 0);
  let start;
  if(C.carryHeat == null || careerStartsFull()) start = cap;
  else start = Math.max(0, Math.min(cap, (C.carryHeat | 0) + careerHeatRecovery()));

  if(start < cap && draw && typeof makeCard === "function"){
    let sample = null; try { sample = makeCard("heat"); } catch(e){}
    const canMint = sample && (typeof isHeat !== "function" || isHeat(sample));
    if(canMint){
      const need = cap - start;
      p.engine = start;
      draw.push(sample);
      for(let i = 1; i < need; i++) draw.push(makeCard("heat"));
      const sh = shuffle(draw.slice()); draw.length = 0; for(const c of sh) draw.push(c);
      if(!p.isBot)
        log(`\uD83D\uDD25 Engine still warm from the last race \u2014 you start on ${start}/${cap} Heat; ${need} burnt Heat ride${need===1?"s":""} in the deck.`, "warn");
    } else {
      p.engine = cap;   // couldn't safely mint Heat cards — leave it full
    }
  } else {
    p.engine = cap;
    if(!p.isBot && C.carryHeat != null && !careerStartsFull())
      log(`\u2744\uFE0F Engine fully cooled \u2014 you line up on a full ${cap} Heat.`, "me");
  }
}
...
const CTYPE = {
  ...
  merchant:{icon:"🛠️",name:"Speed shop",blurb:"Buy Upgrade cards and mods for your car — and pay them to cut a card back out."},
...
const CNODE_R = { start:20, boss:32, elite:25, city:22, enemy:18, drag:18, grudge:21, trial:19, night:20,
  event:18, merchant:19, chop:19, treasure:19, rest:19, town:18, poi:16, legend:24 };
...
function loadCareerSave(){
  try{
    ...
    c.stressStrip=c.stressStrip|0;                // ordinary Stress cards stripped (cut market / events)
    c.curses=Array.isArray(c.curses)?c.curses:[]; // curse-card keys carried in the deck
    c.starterCuts=Array.isArray(c.starterCuts)?c.starterCuts:[];   // stock Speed cards cut out
    if(c.carryHeat===undefined) c.carryHeat=null;                  // null → start next race full
...
function newCareer(rawMap){
  ...
  G.career = { v:2, book, mapName:book.name, act:0,
    ...
    chops:0, stressCuts:0, stressStrip:0, curses:[], tuning:null, table:{ act:1, rows:[] }, starterCuts:[], carryHeat:null,
...
/* Act over — wipe the road behind you and open the next map screen. Money,
   upgrades, mods and the Engine's carried Heat all carry across; only the
   map resets. */
...
function showCareerSelect(){
  ...
  el.innerHTML=`<div class="sheet rules">
    ...
    <div class="tag">Pick your route across the map. Prize money keeps you rolling, the Grand Final crowns you.</div>
    ...
    <div class="phase-hint" style="margin-top:14px">Three acts, one road. Travel is free; the big races charge an entry fee. Races set a goal — win, podium, top-5, lead lap 1, beat a named rival. Hit the goal to claim the stop and bank the purse; miss it and a backer loses patience (and you can pay to re-enter, or route around). Lose the last backer and the season's over.</div>
...
function careerGenMap(){
  ...
  for(let attempt=0;attempt<40;attempt++){
    ...
    const mk=(f,l)=>{ const k=f+","+l; if(grid[k]) return grid[k];
      const n={id:"n"+(nextId++),type:"enemy",label:"",floor:f,x:Math.round(lp(l)+Math.random()*30-15),y:Math.round(fp(f)),data:null};
...
function showCareerMap(){
  ...
  const edgeSvg=C.map.edges.map(([a,b])=>{
    ...
    // hand-drawn feel: bow each edge sideways by a fixed (seeded) amount, and
    // draw un-walked routes as round dots the way Slay the Spire does.
    const mx=(A.x+B.x)/2, my=(A.y+B.y)/2, dx=B.x-A.x, dy=B.y-A.y, len=Math.hypot(dx,dy)||1;
    const wob=((careerHash(a+"~"+b)%1000)/1000-0.5)*Math.min(46,len*0.35);
    const cx=(mx-dy/len*wob).toFixed(1), cy=(my+dx/len*wob).toFixed(1);
    const col=walked?"#8a5cf6":live?"#c9b46b":"#453d5e";
    return `<path d="M${A.x} ${A.y} Q${cx} ${cy} ${B.x} ${B.y}" fill="none" stroke="${col}" stroke-width="${walked?6:3.5}" stroke-linecap="round" stroke-dasharray="${walked?"none":"2 13"}" opacity="${walked||live?0.95:0.4}"/>`;
  }).join("");
  ...
  /* a Slay-the-Spire-style key, top-right, listing only the icons on this map */
  const legendOrder=["enemy","drag","elite","grudge","trial","night","event","merchant","chop","rest","treasure","city","town","poi","boss","legend"];
  const legendHTML=`<div class="cmaplegend" style="position:absolute;top:8px;right:8px;background:rgba(20,15,34,.82);border:1px solid #4a4260;border-radius:10px;padding:7px 9px;font-size:11px;line-height:1.5;color:#f0e7d3;backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px);pointer-events:none;max-width:44%;z-index:2">
    <div style="font-weight:800;letter-spacing:.06em;opacity:.65;margin-bottom:3px">KEY</div>
    ${legendOrder.filter(t=>C.map.nodes.some(n=>n.type===t)).map(t=>`<div>${CTYPE[t].icon} ${CTYPE[t].name}</div>`).join("")}</div>`;
  ...
  /* Slay-the-Spire feel: don't crush the whole act onto one screen. Open zoomed
     to full lane-width on the car (or the start line), sitting low so the road
     ahead is visible, and let the player pan up. ⤢ still fits the whole map. */
  const focusN=(C.at&&cnById(C.at))||C.map.nodes.find(n=>n.type==="start");
  if(focusN) careerMapCam.focusOn(focusN.x, focusN.y, 0.72);
...
function showCareerNodeDetail(node){
  ...
  if(isRace){
    ...
    inner+=`<div class="crow">Cut cards out of your deck for <b>$${careerChopPrice(CHOP_NODE_X)}</b> apiece — drafted Upgrades or the stock Speed cards${(C.chops|0)?` (you've had ${C.chops} done — the price climbs)`:""} — or pay for permanent Stress surgery. Your deck: <b>${careerDeckCount()}</b> cards.</div>`;
  } else if(node.type==="merchant"){
    inner+=`<div class="crow">Upgrade cards and mods on the shelf — and they'll cut a card out of your deck for <b>$${careerChopPrice(CHOP_SHOP_X)}</b> if you ask nicely.</div>`;
...
function showCareerGarage(){
  ...
  box.innerHTML=`<div class="cdetail"><h3>🛠️ ${esc(classByKey(C.cls).name)} — ${defs.length} Upgrade${defs.length===1?"":"s"} fitted</h3>
    <div class="crow" style="color:#bdb3d4">Deck: <b>${careerDeckCount()}</b> cards before Stress${(C.chops|0)?` · ${C.chops} cut out at chop shops`:""}${((C.stressCuts|0)+(C.stressStrip|0))?` · −${(C.stressCuts|0)+(C.stressStrip|0)} Stress`:""}${(C.curses&&C.curses.length)?` · <span style="color:#e08a8a">${C.curses.length} curse${C.curses.length===1?"":"s"} 💀</span>`:""}</div>
  ...
  el.innerHTML=`<div class="sheet" style="max-width:480px">
    ...
    <div class="tracklist" style="margin-top:6px">
      ...
      <button class="trackcard" id="anyShop"><div class="tc-art" style="display:flex;align-items:center;justify-content:center;font-size:34px">🛠️</div>
        <div class="tc-body"><div class="tc-name">Hit the parts market</div><div class="tc-meta">Upgrade cards and mods at street prices.</div></div><div class="tc-go">Shop ▸</div></button>
    </div></div>`;
...
/* ---- the cool-down bay ----------------------------------------------
   Heat now carries between races (only 1 seeps back on its own). A stop here
   lets the Engine cool right down, so you line up next time on a full tank
   instead of dragging burnt Heat through your deck. Free — it's just time. */
function careerHeatServiceHTML(){
  const C=G.career;
  if(C.carryHeat==null || careerStartsFull())
    return `<div class="phase-hint" style="min-height:0">The Engine's stone cold \u2014 nothing to bleed off. You'll start your next race on a full tank.</div>`;
  return `<div class="crow">The Engine's carrying <b style="color:#7fd4ff">${C.carryHeat|0} Heat</b> from your last race. Leave it overnight and it cools right down \u2014 next race you line up on a full Engine instead of a deck clogged with burnt Heat.</div>
    <div class="btnrow"><button class="act" id="chCool">Cool the Engine \u2014 free</button></div>`;
}
...
/* pull one stock Speed card (value 1–4) out of the career deck, for money */
function careerStarterCut(val, price, done){
  const C=G.career;
  if(careerStarterRemaining().length<=STARTER_MIN){ toast("The stock deck's as thin as it goes"); return; }
  if(!careerStarterRemaining().includes(val)){ toast(`No stock Speed ${val} left to cut`); return; }
  if(C.money<price){ toast("You can't cover that cut"); return; }
  C.money-=price; (C.starterCuts=C.starterCuts||[]).push(val); C.chops=(C.chops|0)+1;
  saveCareer();
  toast(`🔧 Stock Speed ${val} cut out — $${price}. Deck: ${careerDeckCount()} cards`);
  done&&done();
}
/* strip one ordinary Stress card out of the deck for good (per-card, unlike the
   pricey two-shot surgery — this is the everyday cut, and it climbs like any) */
function careerStressStrip(price, done){
  const C=G.career;
  if(careerStressRemovable()<=0){ toast("No Stress cards left to pull"); return; }
  if(C.money<price){ toast("You can't cover that cut"); return; }
  C.money-=price; C.stressStrip=(C.stressStrip|0)+1; C.chops=(C.chops|0)+1;
  careerSyncStressTuning(); saveCareer();
  toast(`🔧 A Stress card cut out — $${price}. You start every race with ${(C.stressCuts|0)+(C.stressStrip|0)} fewer.`);
  done&&done();
}
/* cut one curse card out of the deck, for money */
function careerCurseCut(idx, price, done){
  const C=G.career;
  const cur=careerCurseByKey((C.curses||[])[idx]); if(!cur) return;
  if(C.money<price){ toast("You can't cover that cut"); return; }
  C.money-=price; C.curses.splice(idx,1); C.chops=(C.chops|0)+1;
  saveCareer();
  toast(`🔧 ${cur.icon} ${cur.name} cut out for good — $${price}.`);
  done&&done();
}
...
function careerChopMarketHTML(mult){
  ...
  const hasUpg=!!C.upgrades.length;
  return `${hasUpg
      ? `<div class="olbl" style="margin:4px 0 2px">Drafted Upgrade cards</div><div class="draftmkt" id="chopMkt"></div>`
      : `<div class="phase-hint" style="min-height:0">No drafted Upgrade cards in the deck yet — but you can still thin the stock Speed cards below.</div>`}
    <div class="olbl" style="margin:10px 0 2px">Stock Speed cards</div>
    <div id="chopStarter" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:2px"></div>
    ${careerStressRemovable()>0
      ? `<div class="olbl" style="margin:10px 0 2px">Stress cards</div>
         <div id="chopStress" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:2px"></div>`
      : ""}
    ${(C.curses&&C.curses.length)
      ? `<div class="olbl" style="margin:10px 0 2px">Curse cards — dead weight you can't discard</div>
         <div id="chopCurse" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:2px"></div>`
      : ""}
    <div class="phase-hint" style="min-height:0;margin-top:6px">Next cut: <b style="color:${C.money>=price?"#ffd86b":"#e08a8a"}">$${price}</b>${(C.chops|0)?` · ${C.chops} already cut out — the price rises each time`:""}. Deck: <b>${careerDeckCount()}</b> cards before Stress${(C.curses&&C.curses.length)?` · ${C.curses.length} curse${C.curses.length===1?"":"s"}`:""}.</div>`;
}
function careerWireChopMarket(mult, refresh){
  ...
  /* stock Speed cards — plain styled tiles, so we never lean on a game.js
     speed-card renderer we can't see from here */
  const sc=$("#chopStarter");
  if(sc){
    const rem=careerStarterRemaining();
    const atFloor=rem.length<=STARTER_MIN;
    const counts={}; rem.forEach(v=>counts[v]=(counts[v]||0)+1);
    [1,2,3,4].forEach(v=>{
      const n=counts[v]||0; if(!n) return;
      const can=!atFloor && C.money>=price;
      const tile=document.createElement("button");
      tile.type="button";
      tile.style.cssText="display:inline-flex;flex-direction:column;align-items:center;gap:3px;padding:8px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.16);background:rgba(127,212,255,.08);color:inherit;font:inherit;min-width:60px;cursor:"+(can?"pointer":"default")+(can?"":";opacity:.45");
      tile.innerHTML=`<span style="font-size:22px;font-weight:800;line-height:1">${v}</span><span style="font-size:11px;color:var(--cream-dim)">Speed · ×${n}</span><span style="font-size:11px;color:${can?"#ffd86b":"#e08a8a"}">cut $${price}${can?"":" ✗"}</span>`;
      if(can) tile.onclick=()=>careerStarterCut(v, price, refresh);
      sc.appendChild(tile);
    });
    if(atFloor){
      const note=document.createElement("div");
      note.className="phase-hint"; note.style.minHeight="0"; note.style.width="100%";
      note.textContent=`The stock deck's down to ${rem.length} Speed cards — that's as thin as it's safe to run.`;
      sc.appendChild(note);
    }
  }
  /* Stress cards — one tile, showing how many are left to pull */
  const st=$("#chopStress");
  if(st){
    const left=careerStressRemovable(), can=C.money>=price;
    const tile=document.createElement("button");
    tile.type="button";
    tile.style.cssText="display:inline-flex;flex-direction:column;align-items:center;gap:3px;padding:8px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.16);background:rgba(180,140,255,.10);color:inherit;font:inherit;min-width:60px;cursor:"+(can?"pointer":"default")+(can?"":";opacity:.45");
    tile.innerHTML=`<span style="font-size:22px;font-weight:800;line-height:1">＋</span><span style="font-size:11px;color:var(--cream-dim)">Stress · ${left} left</span><span style="font-size:11px;color:${can?"#ffd86b":"#e08a8a"}">cut $${price}${can?"":" ✗"}</span>`;
    if(can) tile.onclick=()=>careerStressStrip(price, refresh);
    st.appendChild(tile);
  }
  /* Curse cards — one tile per curse carried */
  const cc=$("#chopCurse");
  if(cc){
    (C.curses||[]).forEach((key,idx)=>{
      const cur=careerCurseByKey(key); if(!cur) return;
      const can=C.money>=price;
      const tile=document.createElement("button");
      tile.type="button"; tile.title=careerCurseDesc(cur);
      tile.style.cssText="display:inline-flex;flex-direction:column;align-items:center;gap:3px;padding:8px 12px;border-radius:12px;border:1px solid rgba(224,138,138,.4);background:rgba(224,138,138,.10);color:inherit;font:inherit;min-width:64px;cursor:"+(can?"pointer":"default")+(can?"":";opacity:.45");
      tile.innerHTML=`<span style="font-size:20px;line-height:1">${cur.icon}</span><span style="font-size:10.5px;color:var(--cream-dim);text-align:center">${esc(cur.name)}</span><span style="font-size:11px;color:${can?"#ffd86b":"#e08a8a"}">cut $${price}${can?"":" ✗"}</span>`;
      if(can) tile.onclick=()=>careerCurseCut(idx, price, refresh);
      cc.appendChild(tile);
    });
  }
}
function showCareerChopShop(node){
  ...
  el.innerHTML=`<div class="sheet">
    ...
    ${careerRepairHTML()}
    <div class="olbl" style="margin:12px 0 2px">Cool-down bay</div>
    ${careerHeatServiceHTML()}
  ...
  const cb=$("#chCool"); if(cb) cb.onclick=()=>{ G.career.carryHeat=null; saveCareer(); toast("\u2744\uFE0F Engine cooled — next race starts on a full tank"); showCareerChopShop(node); };
  const sb=$("#chStress");
  if(sb && sp!=null && C.money>=sp) sb.onclick=()=>{
    C.money-=sp; C.stressCuts=(C.stressCuts|0)+1;
    careerSyncStressTuning();
    saveCareer();
...
/* Build the list of cards a career can pay/choose to pull out of the deck:
   drafted Upgrades, stock Speed cards (down to the floor), Stress cards, and
   Curse cards. Each entry knows how to render a tile and how to remove itself.
   `mode` "curse" narrows it to curses only (the exorcise event). */
function careerRemovableList(mode){
  const C=G.career, out=[];
  const onlyCurse = mode==="curse";
  if(!onlyCurse){
    (C.upgrades||[]).forEach((id,idx)=>{
      const def=upgradeById(id); if(!def) return;
      out.push({ label:def.name, node:()=>renderCard(makeUpgCard(def)),
        remove:()=>{ C.upgrades.splice(idx,1); } });
    });
    const rem=careerStarterRemaining(), counts={}; rem.forEach(v=>counts[v]=(counts[v]||0)+1);
    if(rem.length>STARTER_MIN){
      [1,2,3,4].forEach(v=>{ if(!counts[v]) return;
        out.push({ label:`Stock Speed ${v}`, tile:{txt:String(v), sub:`Speed · ×${counts[v]}`, bg:"rgba(127,212,255,.08)"},
          remove:()=>{ (C.starterCuts=C.starterCuts||[]).push(v); } });
      });
    }
    if(careerStressRemovable()>0){
      out.push({ label:"Stress card", tile:{txt:"＋", sub:`Stress · ${careerStressRemovable()} left`, bg:"rgba(180,140,255,.10)"},
        remove:()=>{ C.stressStrip=(C.stressStrip|0)+1; careerSyncStressTuning(); } });
    }
  }
  (C.curses||[]).forEach((key,idx)=>{
    const cur=careerCurseByKey(key); if(!cur) return;
    out.push({ label:cur.name, curse:true, tile:{txt:cur.icon, sub:cur.name, bg:"rgba(224,138,138,.10)", desc:careerCurseDesc(cur)},
      remove:()=>{ C.curses.splice(idx,1); } });
  });
  return out;
}
/* one tap = pull one card out of the deck. Handles Upgrades, Speed, Stress and
   Curses; `mode:"curse"` (the exorcise event) shows only curses. */
function showCareerScrapPick(title, body, reward, done, mode){
  const C=G.career;
  const list=careerRemovableList(mode);
  if(!list.length){
    if(mode==="curse"){ careerNotice(title, "There's no bad luck in this car to clear — it's clean.", "", done); return; }
  ...
  el.innerHTML=`<div class="sheet"><h2>${title}</h2>
    ...
    <div class="draftmkt" id="scrapPick" style="display:flex;flex-wrap:wrap;gap:8px;align-items:flex-start"></div>
    ...
    <div class="btnrow" style="margin-top:12px"><button class="act secondary" id="scrapSkip">${mode==="curse"?"Leave it be":"Keep them all"}</button></div></div>`;
  const mkt=$("#scrapPick");
  const take=(entry)=>{
    entry.remove();
    if(reward){ C.money+=reward.m||0; if(reward.f) careerRepAdd(reward.f); }
    saveCareer();
    toast(`🗑 ${entry.label} pulled out of the deck for good`);
    done();
  };
  list.forEach(entry=>{
    if(entry.node){
      const cd=entry.node(); cd.onclick=()=>take(entry); mkt.appendChild(cd);
    } else {
      const t=entry.tile, tile=document.createElement("button"); tile.type="button";
      if(t.desc) tile.title=t.desc;
      tile.style.cssText=`display:inline-flex;flex-direction:column;align-items:center;gap:3px;padding:10px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.16);background:${t.bg};color:inherit;font:inherit;min-width:64px;cursor:pointer`;
      tile.innerHTML=`<span style="font-size:22px;font-weight:800;line-height:1">${t.txt}</span><span style="font-size:10.5px;color:var(--cream-dim);text-align:center">${esc(t.sub)}</span>`;
      tile.onclick=()=>take(entry);
      mkt.appendChild(tile);
    }
  });
...
/* ---------------- roadside events
   ...
   Option shape — every field is optional:
     ...
     scrap     true → pull one card back out of your deck (Upgrade, stock
                       Speed, Stress or Curse — see careerRemovableList)
     scrapPay  { m, f } paid for scrapping
     curse     true → add a random curse card; "key" → add that specific one.
                       Lives on an OUTCOME, so a gamble's lose side can curse you.
     cleanse   true → open a curses-only removal picker (the exorcise event)
...
const CAREER_EVENTS=[
  ...
  /* ================= CURSE EVENTS =================
     Each one is a bargain: money or a part now, a dead card in the deck
     forever (or until a chop shop cuts it out). `curse:true` adds a random
     curse; `curse:"key"` names one. Put on an outcome so it can be the losing
     side of a gamble, too. `cleanse:true` opens a curses-only removal picker. */

  {name:"Bargain-Bin Turbo", icon:"🛢️", text:"A backstreet garage will bolt a salvaged turbo onto your car for next to nothing. It rattles. The man swears that's normal.",
    opts:[
      {lbl:"Fit it — grab an Upgrade, but it leaves something nasty in the deck", upg:true, curse:true,
        note:"Free power. The car never feels quite right again."},
      {lbl:"Walk away clean"}
    ]},

  {name:"Haunted Trophy", icon:"🏆", text:"A dead team's championship trophy is going cheap at a car-boot sale. The last three owners all DNF'd their next race. Coincidence, surely.",
    opts:[
      {lbl:"Sell it on for $70 — bad luck clings to the car", m:70, curse:true,
        note:"The cash spends fine. The luck rides with you."},
      {lbl:"Don't tempt fate"}
    ]},

  {name:"Midnight Shortcut", icon:"🌙", text:"An unlit farm track cuts an hour off the run to the next meeting. It also cut the sump out of the last car that tried it.",
    opts:[
      {lbl:"Chance it — 50/50: +$60 saved, or you break something", gamble:{win:{m:60}, lose:{curse:true}}},
      {lbl:"Take the long way round, safe"}
    ]},

  {name:"Cowboy Mechanic", icon:"🔧", text:"A man with one enormous wrench offers to 'sort your car out' for beer money. He does not appear to own any other tools.",
    opts:[
      {lbl:"Let him at it (+$40 saved, but he leaves a curse behind)", m:40, curse:true,
        note:"It's fixed. It's also worse. Both are true."},
      {lbl:"Politely decline"}
    ]},

  {name:"Gypsy Curse", icon:"🃏", text:"You cut off the wrong car leaving the last town. Its driver, an old road-racer with a long memory, says three words and points at your engine.",
    opts:[
      {lbl:"Laugh it off (something's wrong with the car now)", curse:true,
        note:"Probably nothing. The temperature gauge disagrees."},
      {lbl:"Apologise and buy them a drink (−$30, no hard feelings)", m:-30, needM:30}
    ]},

  /* the cure: an exorcise stop. Only useful if you're carrying a curse. */
  {name:"The Pit Priest", icon:"🕯️", text:"An old mechanic who's seen every way a car can betray a driver offers to 'clear the bad luck out' of yours. He'll take a donation, not a fee.",
    opts:[
      {lbl:"Let him work — pull a curse out of the deck for good", cleanse:true,
        note:"He mutters over the engine. Something lifts."},
      {lbl:"Tip him anyway (+1 ⭐), leave the car as it is", rep:1},
      {lbl:"You don't believe in that sort of thing"}
    ]}
];
...
function renderCareerEvent(node, ev, depth){
  ...
  const apply=(o)=>{
    ...
    let addedCurse=null;
    if(outcome.curse){ addedCurse=careerAddCurse(outcome.curse===true?null:outcome.curse); }
    ...
    if(addedCurse) bits.push(`${addedCurse.icon} ${addedCurse.name} (curse)`);
    ...
    if(o.cleanse){
      showCareerScrapPick(`${ev.icon||"❓"} ${ev.name}`,
        o.note || "Which bit of bad luck leaves the car?", null, next, "curse");
      return;
    }
...
function startCareerRace(node,cfg){
  ...
  for(const h of humans()){ careerPrepHumanCar(h); applyWeatherCarSetup(h); drawToHand(h,handSize()); }
...
function careerRaceOver(order){
  ...
  const repDelta = ok ? (cfg.bonus.rep||0) : -1;
  const repMoved = careerRepAdd(repDelta);
  /* ---- Engine heat carries into the next race ------------------------
     Read the human's Engine before the derived classes are stripped, so
     heatCapFor still reports this car's true capacity. carryHeat is stored
     raw; startCareerRace re-caps it against next race's Engine size. */
  const endEngine = Math.max(0, (you.engine|0));
  const heatCap   = (typeof heatCapFor==="function") ? heatCapFor(you) : endEngine;
  const startsFull = careerStartsFull();
  const nextHeat  = startsFull ? heatCap : Math.max(0, Math.min(heatCap, endEngine + careerHeatRecovery()));
  C.carryHeat = endEngine;
  ...
  showCareerRaceResult(node,cfg,order,you,pos,ok,gainM,repMoved,wonOutright,
                       { spins, dmgGained, impounded, grudge:gfoe,
                         heat:{ end:endEngine, next:nextHeat, cap:heatCap, full:startsFull, recovery:careerHeatRecovery() } });
}
/* ---- prizes --------------------------------------------------------
...
function showCareerRaceResult(node,cfg,order,you,pos,ok,gainM,repMoved,wonOutright,after){
  ...
  el.innerHTML=`<div class="sheet">
    ...
    ${after.heat?`<div class="crow" style="justify-content:center;color:#7fd4ff">\uD83C\uDF21\uFE0F ${after.heat.full
      ? `Serviced Engine \u2014 next race starts on a full <b>${after.heat.cap}</b> Heat.`
      : `Engine holds <b>${after.heat.end}</b> Heat at the flag; <b>+${after.heat.recovery}</b> seeps back overnight \u2014 you line up next race on <b>${after.heat.next}/${after.heat.cap}</b>, the rest riding burnt in your draw pile.${after.heat.end<=0?" A chop-shop cool-down would clear it.":""}`}</div>`:""}
