...
/* =====================================================================
   ...
   _fx fields the hooks understand:
     ...
     forgive     +n   over-limit by <= n costs nothing  [folded into limitAdj]
     ...
     postHeat    +n   extra Heat the Engine recovers between races (career.js)
     startFull   bool ignore between-race carry-over: Engine starts full

   Career-layer fields live under _fx.career and are read by career.js:
     money         paid after every race
     ...
     retryHalf     re-entering a missed race is free (no entry fee)
   (Fuel is gone: the old fuel / fuelDiscount / restBonus fields no longer
    exist on any mod. Between-race Engine recovery is postHeat / startFull,
    handled by career.js — see careerHeatRecovery there.)
   ===================================================================== */
...
const CAREER_MODS = [
  ...
  { id:"bigbore", name:"Big-Bore Radiator", icon:"\uD83D\uDEE2\uFE0F", rarity:"common",
    text:"+2 Heat capacity, and your Engine starts every race full \u2014 no Heat carried over.",
    flavour:"It weighs a ton and you do not care.",
    fx:{ tank:2, startFull:true } },
  ...
  { id:"scavcrate", name:"Scavenger's Crate", icon:"\uD83E\uDDF0", rarity:"common",
    text:"After every race, +$20 — win or lose.",
    flavour:"Half the paddock's spares end up in the back of your truck.",
    fx:{ career:{ money:20 } } },

  { id:"pitcrew", name:"Overnight Pit Crew", icon:"\uD83C\uDF19", rarity:"uncommon",
    text:"Between races the Engine recovers 2 extra Heat — you carry over 3 instead of 1.",
    flavour:"They work through the night so you don't line up hot.",
    fx:{ postHeat:2 } },
  ...
  { id:"coolantsys", name:"Coolant Overhaul", icon:"\u2744\uFE0F", rarity:"common",
    text:"Between races the Engine recovers 1 extra Heat — you carry over 2 instead of 1.",
    flavour:"A bigger header tank and a night to let it all bleed off.",
    fx:{ postHeat:1 } },
  ...
  { id:"paddockpass", name:"Paddock Pass", icon:"\uD83C\uDFAB", rarity:"rare",
    text:"Re-entering a race you missed is free \u2014 no entry fee.",
  ...
  { id:"blueprint", name:"Blueprint Engine", icon:"\uD83D\uDCD0", rarity:"legendary",
    text:"+4 Heat capacity, +1 Cooldown in every gear, and your Engine starts every race brim-full \u2014 no Heat carried over.",
    flavour:"Hand-built to the drawings nobody was ever supposed to see.",
    fx:{ tank:4, cool:1, startFull:true } },
...
/* Career-layer shortcut: careerPerk("shopDisc") -> 0 when no mod grants it. */
...
function careerApplyFx(players){
  ...
  for(const p of (players || [])){
    const fx = fxOf(p);
    if(!fx) continue;
    if(fx.tank || fx.cool || fx.limitAdj || fx.forgive || fx.slipBase || fx.spinStress){
      ...
      const derived = Object.assign({}, base, {
        ...
        /* limitAdj raises the printed corner limit for this driver; forgive
           ("over by <= n is free") is arithmetically the same thing, so it
           rides here too. Folding it in — instead of a live-check-only patch —
           means the higher limit and the lower Heat bill show up identically
           in every forecast the driver is shown (corner strip, confirm gate,
           Boost/Cooldown previews) and in the bot AI's own cost math. */
        limitAdj:   (base.limitAdj||0) + (fx.limitAdj||0) + (fx.forgive||0),
        slipBase:   (base.slipBase||2) + (fx.slipBase||0),
        spinStress: Math.max(-1, (base.spinStress||0) + (fx.spinStress||0)),
        /* base.cool is a per-gear table like {1:3, 2:1}, not an array — map
           over its entries so we return the same {gear:value} shape. */
        cool:       Object.fromEntries(
                      Object.entries(base.cool||{})
                            .map(([g, v]) => [g, Math.max(0, (v||0) + (fx.cool||0))])
                    )
      });
...
(function installCareerHooks(){
  ...
  /* ---- C · runCornerCheck(p) ------------------------------------------
     ...
     · noOverheat    — Overheat corner tokens don't add their +1 for the holder.
     forgive is NOT handled here any more: it now rides in the driver's derived
     limitAdj (see careerApplyFx), so it lands on the live check AND on every
     Heat forecast at once, instead of only the live check. */
  const _rcc = runCornerCheck;
  const _cho = cornerHasOverheat;
  /* Overheat suppression, keyed to the *acting* driver rather than a bare
     "we're mid-check" flag. That one change is what makes the token drop out
     of the numbers the player is shown BEFORE they commit, not just the ones
     paid afterwards:
       · During a corner check, runCornerCheck pins G._ohActor to the driver
         being checked (human or bot), so their check — and only theirs — sees
         the token removed.
       · Outside a check, every Heat forecast (cornerHeatCost previews, the
         confirm gate, Boost/Cooldown estimates) also flows through here; with
         no actor pinned we fall back to the human at the wheel, so the limit
         and Heat bill they're shown already have the token taken off. */
  cornerHasOverheat = function(ct){
    const actor = G._ohActor || (typeof curHuman === "function" ? curHuman() : null);
    const afx = fxOf(actor);
    if(afx && afx.noOverheat) return false;
    return _cho(ct);
  };
  runCornerCheck = function(p){
    ...
    /* Pin the overheat actor for the duration of the real check so
       cornerHasOverheat resolves to THIS driver (forgive already rides in
       p.limitAdj, so there's nothing to bump/restore here any more). */
    const prevActor = G._ohActor;
    G._ohActor = p;
    let r;
    try { r = _rcc(p); } finally { G._ohActor = prevActor; }
    return r;
  };
